...
#####################
    ...
    def flux(self, cMF, perleni, RFe, PT, PE, Eosurf_max, Zr_elev, VEGarea, HEADSini, TopSoilLay, BotSoilLay, Tl, nsl, Sm, Sfc, Sr, Ks, Ssurf_max, Ssoil_ini, Rp_ini, Ssurf_ini, EXF_ini, dgwt, st, i, j, n, kT_min, kT_max, kT_f, kT_s, NVEG, LAIveg):
        ...
        Ssurf_tmp  += Decimal(float(RFe)).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN) + Decimal(Ssurf_ini).quantize(Decimal('.001'), rounding=ROUND_HALF_EVEN)
        # Soil
        Ssoil_tmp = []
        Rexf_tmp =  []
        for l in range(nsl):
        ...
        Tsoil_tmpZr = list(np.zeros([nsl,len(Zr_elev)]))
                    ...
                    if LAIveg[v] > 0.0:
                        if VEGarea[v] > 0.0:
                            if BotSoilLay[l] > Zr_elev[v]:
                                ...
                                PTc = PT[v]*(TopSoilLay[l]-np.float32(Zr_elev[v]/Tl[l]))
        ...
        dgwt_corr_tmp = float(dgwt_corr)*0.1
        ...
        if cMF.wel_yn == 1:
            ...
            for jj, (Zr_elev_, v, kT_min_, kT_max_, kT_f_, kT_s_) in enumerate(zip(np.array(Zr_elev)[order],np.array(list(range(NVEG)))[order], np.array(kT_min)[order], np.array(kT_max)[order], np.array(kT_f)[order], np.array(kT_s)[order])):
                #print '----'
                        ...
                        if np.isclose(Ssoil_pc_tmp[l], cMF.hnoflo):
                            ...
                            if Ssoil_pc_tmp[l] > Sr[l]:
                                if Ssoil_pc_tmp[l] < Sm[l]:
                                    kT = kT_max_- (kT_max_-kT_min_) / (1 + np.exp(Ssoil_pc_tmp[l]-kT_f_/kT_s_))
                                else:
                                    print("WARNING!\nComputing of Tg: soil moisture higher than porosity!\nSoil moisture = %.4f, phi = %.2f"%(Ssoil_pc_tmp[l], Sm[l]))
                                    kT = kT_max_
                            else:
                                print("WARNING!\nComputing of Tg: soil moisture lower than wilting point!\nSoil moisture = %.4f, WP = %.2f"%(Ssoil_pc_tmp[l], Sr[l]))
                                kT = kT_min_
                            Tg_tmp_Zr[l,v] = PT[v]*kT
                            PT[v] -= Tg_tmp_Zr[l,v]
                            Tg_tmp1 = (Tg_tmp_Zr[l,v]*VEGarea[v]*0.01)
                            if Tg_tmp1 > 0 and (HEADSini_corr_tmp-Tg_tmp1/sy_tmp) < Zr_elev_:
...
#                    print 'veg%d: root too short!' % v
        ...
        return Esurf_tmp, Ssurf_tmp, Ro_tmp, Rp_tmp, Esoil_tmp, Tsoil_tmp, Ssoil_tmp, Ssoil_pc_tmp, Eg_tmp, Tg_tmp, HEADSini_corr, dgwt_corr, SAT, Rexf_tmp, I
...
#####################
    ...
    def runMMsoil(self, _nsl, _nslmax, _st, _Sm, _Sfc, _Sr, _slprop, _Ssoil_ini, botm_l0, _Ks,
            ...
            LAI_veg_zonesSP, Zr, kT_min, kT_max, kT_f, kT_s, NVEG,
            cMF, conv_fact, h5_MF, h5_MM, irr_yn,
            RF_irr_zoneSP = [], PT_irr_zonesSP = [], RFe_irr_zoneSP = [],
            crop_irr_SP = [], gridIRR = [],
            Zr_c = [], kT_min_c = [], kT_max_c = [], kT_f_c= [], kT_s_c= [], NCROP = [],
                ...
                for j in range(cMF.ncol):
                    if cMF.outcropL[i,j] > 0:
                       ...
                       # MASS BALANCE COMPUTING
                        MBsurf = RFe_tot + float(Rexf_tmp[0]) - (float(Esurf_tmp) + float(Ro_tmp) + float(I) + float(dSsurf))
                        if nsl > 1:
                            ...
                            for l in llst:
                                ...
                                In =  Rp_ini_tmp_array[i,j,l-1]*perleni + float(Rexf_tmp[l+1]) #
                                Out = float(Rp_tmp[l]) + float(Rexf_tmp[l]) + float(Esoil_tmp[l]) + float(Tsoil_tmp[l]) + dSsoil[l]
                                MB_l[l] = In - Out
                                del In, Out
                            # last soil layer
                            l = nsl-1
                            In = Rp_ini_tmp_array[i,j,l-1]*perleni + exf_MF_ini_tmp*perleni  #*perleni/cMF.perlen[n-1]
                            Out = float(Rp_tmp[l]) + float(Rexf_tmp[l]) + float(Esoil_tmp[l]) + float(Tsoil_tmp[l]) + dSsoil[l]   #float(sum(Rexf_tmp))
                            MB_l[l] = In - Out
                            del In, Out
                        else:
                            # only one soil layer
                            l = 0
                            In = float(I) + exf_MF_ini_tmp*perleni
                            Out = float(Rp_tmp[l]) + float(Rexf_tmp[l]) + float(Esoil_tmp[l]) + float(Tsoil_tmp[l]) +  dSsoil[l]
                            MB_l[l] = In - Out
                            del In, Out
                            # last soil layer
                        # total mass balance for the soil
                        In = float(I) + exf_MF_ini_tmp*perleni + dRp_tot
                        Out = float(Rexf_tmp[0]) + Esoil_MB + Tsoil_MB + dSsoil_tot
                        MB = In - Out
                        del In, Out
                      
                        # export list
                        # indexes of the HDF5 output arrays
                        # index_MM = {'iRF':0, 'iPT':1, 'iPE':2, 'iRFe':3, 'iSsurf':4, 'iRo':5, 'iEXFg':6, 'iEsurf':7, 'iMB':8, 'iEi':9, 'iEo':10, 'iEg':11, 'iTg':12, 'idSsurf':13, 'iETg':14, 'iETsoil':15, 'iSsoil_pc':16, 'idSsoil':17, 'iperc':18, 'ihcorr':19, 'idgwt':20, 'iuzthick':21, 'iI':22, 'iMBsurf':23}
                        MM_tmp = [RF_tmp, PT_tot, PE_tot, RFe_tot, np.float32(Ssurf_tmp), np.float32(Ro_tmp), exf_MF_ini_tmp, np.float32(Esurf_tmp), np.float32(MB), np.float32(INTER_tot), Eo_zonesSP_tmp, np.float32(Eg_tmp), np.float32(Tg_tmp), np.float32(dSsurf), np.float32(ETg), np.float32(ETsoil_tot), np.float32(Ssoil_pc_tot), np.float32(dSsoil_tot), np.float32(perc), np.float32(HEADSini_MM)*0.001, np.float32(-dgwt_tmp)*0.001, uzthick*0.001, np.float32(I), MBsurf]
