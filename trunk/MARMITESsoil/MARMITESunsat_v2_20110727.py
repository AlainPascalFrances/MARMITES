...
#####################
    ...
    def flux(self, RFe, PET, PE, Su, Rp, Zr_elev, VEGarea, HEADS, HEADS_ini, Dltop, Dlbot, Dl, Dl_ini, nsl, Sm, Sfc, Sr, Ks, Ss_max, Ss_ratio, t, Su_ini, Rp_ini, DRN, DRN_ini, Ss_ini, E0, dtwt, st, i, j, n, kTu_min, kTu_n, dt, dti):
        ...
        def surfwater(s_tmp,Sm,Ss_max, E0, i, j, n, dt):
            ...
            if s_tmp > Sm:
                Ss_tmp = s_tmp-Sm
                if Ss_tmp > Ss_max or np.abs(Ss_tmp - Ss_max) < 1E-6:
                    Ro_tmp = (Ss_tmp-Ss_max)/dt
                    Ss_tmp = Ss_max
                else:
                    Ro_tmp = 0.0
                if Ss_tmp > E0 or np.abs(Ss_tmp - E0) < 1E-6:
                    Es_tmp = E0
                    Ss_tmp = Ss_tmp - E0*dt
                else:
                    Es_tmp = Ss_tmp/dt
        ...
        def perc(s_tmp,Sm,Sfc,Ks, s_lp1, Sm_sp1, i, j, n, dt):
            ...
            '''
            global rp_tmp
            if Sfc > s_tmp or np.abs(s_tmp-Sfc) < 1E-6:
                rp_tmp = 0.0
            elif s_tmp > Sfc and ( Sm > Sfc or np.abs(s_tmp-Sm) < 1E-6):
        ...
        def evp(s_tmp,Sfc,Sr, pet, i, j, n, dt):
            ...
            global evp_tmp
            if (s_tmp < Sr) or np.abs(s_tmp - Sr) < 1E-6:
                evp_tmp = 0.0
            elif (s_tmp > Sr) and ((s_tmp < Sfc) or np.abs(s_tmp -Sfc) < 1E-6):
                Se = (s_tmp - Sr)/(Sfc - Sr)
                if Se > 1.0:
                    sys.exit()
                if (pet*Se*dt) > (s_tmp - Sr):
                    evp_tmp = (s_tmp - Sr)/dt
                else:
                    evp_tmp = pet*Se
            elif s_tmp > Sfc :
        ...
        if DRN > 0.0:
##            if DRN_ini == 0.0:
##                Su_tmp[nsl-2] += Su_tmp[nsl-1]-Sr[nsl-1]*Dl_ini[nsl-1]
##                Su_tmp[nsl-1] = Sr[nsl-1]*Dl_ini[nsl-1]
##                #GW_corr += Sr[nsl-1]*Dl_ini[nsl-1]
##                if i == 10 and j == 4:
##                    print 'A', ' INI %.4f' % (HEADS_ini/1000), 'ACT %.4f' % (HEADS/1000), 'DIFF %.4f' % (HEADS_ini/1000 - HEADS/1000), 'CORR %.4f' % (GW_corr), '\n'
##                for l in llst:
...
##        else:
##            if DRN_ini > 0.0:
##                Su_tmp[nsl-1] += Dl[nsl-1]*Sfc[nsl-1]
##                #GW_corr -= Dl[nsl-1]*(Sfc[nsl-1]-Sr[nsl-1])
####                if i == 10 and j == 4:
####                    print 'B', ' INI %.4f' % (HEADS_ini/1000), 'ACT %.4f' % (HEADS/1000), 'DIFF %.4f' % (HEADS_ini/1000 - HEADS/1000), 'CORR %.4f' % (GW_corr), '\n'
##            else:
##                if HEADS_ini > HEADS:
##                    Su_tmp[nsl-1] += (HEADS_ini - HEADS)*Sfc[nsl-1]
##                    #GW_corr -= (HEADS_ini - HEADS)*(Sfc[nsl-1]-Sr[nsl-1])
####                    if i == 10 and j == 4:
####                        print 'C', ' INI %.4f' % (HEADS_ini/1000), 'ACT %.4f' % (HEADS/1000), 'DIFF %.4f' % (HEADS_ini/1000 - HEADS/1000), 'CORR %.4f' % (GW_corr), '\n'
##                else:
##                    #GW_corr += (HEADS - HEADS_ini)*Sr[nsl-1]
####                    if i == 10 and j == 4:
####                        print 'D', ' INI %.4f' % (HEADS_ini/1000), 'ACT %.4f' % (HEADS/1000), 'DIFF %.4f' % (HEADS_ini/1000 - HEADS/1000), 'CORR %.4f' % (GW_corr), '\n'
##                    for l in llst:
##                        if Su_tmp[l] > Sm[l]*Dl[l]:
##                            Su_tmp[l-1] += Su_tmp[l] - Sm[l]*Dl[l]
##                            Su_tmp[l] = Sm[l]*Dl[l]
        ...
        for l in llst_full[:-1]:
            ...
            if Rpprev > (Sm[l]*Dl[l]-Su_tmp[l]):
                ...
                Su_tmp[l-1] += Rpprev
                if l > 1:
                    if (Su_tmp[l-1] > Sm[l-1]*Dl[l-1]):
                        Su_tmp[l-2] += Su_tmp[l-1] - Sm[l-1]*Dl[l-1]
                        Su_tmp[l-1] = Sm[l-1]*Dl[l-1]
            else:
                Su_tmp[l] += Rpprev
        ...
        for l in llst_full:
            if DRN_ini_TS > 0.0  and np.abs(Su_tmp[l] - (Sm[l]*Dl[l])) < 1E-6:
                HEADS += Dl[l]
                dtwt -= Dl[l]
                SAT[l] = True
            else:
        ...
        for l in range(nsl):
            if SAT[l] == False:
                ...
                if Ss_tmp == 0.0:
                    if PE > 0.0:
                        Eu_tmp[l] = evp(Su_tmp[l],Sfc[l]*Dl[l],Sr[l]*Dl[l], PE, i, j, n, dt)
                ...
                for z in range(len(Zr_elev)):
                    if PET[z] > 0.0 :
                        if Dlbot[l] > Zr_elev[z]:
                            Tu_tmpZr[l,z] = evp(Su_tmp[l],Sfc[l]*Dl[l],Sr[l]*Dl[l], PET[z], i, j, n, dt)
                        elif Dltop[l] > Zr_elev[z] :
                            PETc = PET[z]*(Dltop[l]-Zr_elev[z])/Dl[l]
                            Tu_tmpZr[l,z] = evp(Su_tmp[l],Sfc[l]*Dl[l],Sr[l]*Dl[l], PETc, i, j, n, dt)
        ...
        return Es_tmp, Ss_tmp, Ro_tmp, Rp_tmp, Eu_tmp, Tu_tmp, Su_tmp, Su_pc_tmp, Su_ini, R_tmp, Eg_tmp, Tg_tmp, HEADS, dtwt, GW_corr
...
#####################
    ...
    def run(self, i, j, n,
        ...
        # HEADS
        HEADS_corr=np.zeros([Ttotal], dtype=int)
        # GW correction
        GW_corr=np.zeros([Ttotal], dtype=int)
        #MASS BALANCE
...
#            D=0.05
        ...
        for t in range(int(nstp)):    # t: current time step
            ...
            MB[t] = RFe_tot[t] + Ss_ini + dti*Rp_in_MB + DRN_tmp  - GW_corr[t] - Ss[t] - dt*(Ro[t] + Es[t] + Eu_MB + Tu_MB + Rp_out_MB + dSu_tot[t])
