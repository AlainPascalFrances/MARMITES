...
#####################
    ...
    def run(self, _nsl, _nslmax, _st, _Sm, _Sfc, _Sr, _slprop, _Ssoil_ini, botm_l0, _Ks,
        ...
        global h_MF, dti, MM_S, MM, thti, imfRCH, imfEXF, IRRfield, h_MF_l
        JD_nper = []
            ...
            for i in range(cMF.nrow):
                for j in range(cMF.ncol):
                    if cMF.iuzfbnd[i][j] != 0.0:
                        ...
                        dt = float(cMF.perlen)/float(cMF.nstp)

                        # PROCESSING THE WHOLE DATA SET
                        for t in range(int(cMF.nstp)):    # t: current time step
...
#                                print 'Alleluia, EXF at i:%02d j:%03d n:%03d' % (i,j,n)
                            ...
                            if nsl > 1:
                                ...
                                # last soil layer
                                l = nsl-1
                                MB_l[t,l] = (Rp_ini_tmp_array[i,j,l-1]*dti - Rp[t,l]*dt)/dt + dSsoil[t,l] + exf_MF[i,j] - (Esoil[t,l] + Tsoil[t,l] + Rexf[t,l])
                            else:
                                # only one soil layer
                                l = 0
                                MB_l[t,l] = (RFe_tot[t] + dSsoil[t,l] + dSsurf[t] + exf_MF[i,j]) - (Rp[t,l] + Esoil[t,l] + Tsoil[t,l] + Ro_tmp + Esurf_tmp)
                                # last soil layer
                            # total mass balance for the soil
                            MB[t] = RFe_tot[t] + dSsurf[t] + dSsoil_tot[t] + dRp_tot + exf_MF[i,j] - (Ro[t] + Esurf[t] + Esoil_MB + Tsoil_MB)
                            # export list
                            ...
                            MM_tmp[t,:] = [RF[t], PT_tot[t], PE_tot[t], RFe_tot[t], Ssurf[t], Ro[t], exf_MF[i,j], Esurf[t], MB[t], INTER_tot[t], E0[t], Eg[t], Tg[t], dSsurf[t], ETg[t], ETsoil_tot[t], Ssoil_pc_tot[t], dSsoil_tot[t], inf[t], HEADS_MM[t]*0.001, -dgwt[t]*0.001, uzthick[t]*0.001]
                        ...
                        if (float(cMF.perlen)/float(cMF.nstp)) != 1.0:
                            for stp in range(cMF.nstp):
                                ts = float(cMF.perlen)/float(cMF.nstp)
                                tstart =    stp*ts
                                tend   =   (1+stp)*ts
                        ...
                        Ssoil_ini_tmp_array[i,j,:]  = MM_S[cMF.nstp-1,i,j,:,index_S.get('iSsoil')]
                        Rp_ini_tmp_array[i,j,:]  = MM_S[cMF.nstp-1,i,j,:,index_S.get('iRp')]
                        Ssurf_ini_tmp_array[i,j]    = MM[cMF.nstp-1,i,j,index.get('iSsurf')]
                    else:
                        if cMF.perlen>1:
                            MM[:,i,j,:] = cMF.hnoflo
                            MM_S[:,i,j,:,:] = cMF.hnoflo
                        ...
                        MM_finf_MF[i,j] = 0.0
                        MM_wel_MF[i,j] = 0.0
            dti = float(cMF.perlen)/float(cMF.nstp)
            ...
            # run MODFLOW with Rp
            try:
                cMF.ppMF(MF_ws, MF_ini_fn, finf_MM = h5_MM['finf'][n,:,:], wel_MM = h5_MM['ETg'][n,:,:], MMsoil = 1, silent = 1)
            except:
                raise SystemExit('\nMODFLOW error in MMsoil computing!\nCheck the MODFLOW list file in folder:\n%s\nError description:\n%s' % (MF_ws, traceback.print_exc(file=sys.stdout)))
