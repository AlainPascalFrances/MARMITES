...
#####################
    ...
    def flux(self, RFe, PET, PE, Su, Rp, Zr_elev, VEGarea, HEADS_tmp, Dltop, Dlbot, Dl, Dl_ini, nsl, Sm, Sfc, Sr, Ks, Ss_max, Ss_ratio, t, Su_ini, Rp_ini, DRN, DRN_ini, Ss_ini, E0, dtwt, st, i, j, n, kTu_min, kTu_n, dt, dti):
...
##        ##################
        ...
        for l in llst_full:
            if Dl[l]> 0.0  and Su_tmp[l] == (Sm[l]*Dl[l]): # > 1E-9:
                HEADS_tmp += Dl[l]
                dtwt -= Dl[l]
                SAT[l] = True
            else:
        ...
        for l in range(nsl):
            ...
            if l < (nsl-1):
                ...
                if Dl[l] > 0.0 and DRN_corr == 0.0:
                    ...
                    R_tmp = Rp_tmp[l]
            if SAT[l] == False:
                if (Dlbot[l] - HEADS_tmp) > 1E-9:
                    Dl_tmp = Dl[l]
                elif (Dltop[l] - HEADS_tmp) > 1E-9:
                    Dl_tmp = Dltop[l] - HEADS_tmp
                if Dl[l] > 0.0:
                    # Eu
                    if Ss_tmp == 0.0:
                        if PE > 0.0:
        ...
        for l in range(nsl):
            if Dl[l] > 0.0:
##                if i==8 and j==2 and l==2 and DRN_ini > 0.0 and DRN == 0.0:
##                    print ''
##                    print 'n ', n
##                    print 'DRN_ini ', DRN_ini
##                    print 'DRN ', DRN
##                    print 'Su ', Su_tmp[l]
##                    print 'Dl ', Dl[l]
##                    print 'Su_pc ', Su_pc_tmp[l]
                Su_pc_tmp[l] = Su_tmp[l]/Dl[l]
##                if i==8 and j==2 and l==2 and DRN_ini > 0.0 and DRN == 0.0:
##                    print 'Su_pc2 ', Su_pc_tmp[l]
            else:
                Su_pc_tmp[l] = Sm[l]
                Su_tmp[l] = Sm[l]*Dl[l]
        ...
        for z in range(len(Zr_elev)):
            if ((HEADS_tmp - Dlbot[nsl-1]) > 1E-9) and ((HEADS_tmp - Zr_elev[z]) > 1e-9):
                for l in range(nsl):
                    if Su_pc_tmp[l] >= Sr[l]:
                        if Su_pc_tmp[l] <= Sm[l]:
        ...
        return Es_tmp, Ss_tmp, Ro_tmp, Rp_tmp, Eu_tmp, Tu_tmp, Su_tmp, Su_pc_tmp, R_tmp, Eg_tmp, Tg_tmp, HEADS_tmp, DRN_corr
        del Es_tmp, Ss_tmp, Ro_tmp, Rp_tmp, Eu_tmp, Tu_tmp, Su_tmp, Su_pc_tmp, R_tmp, Eg_tmp, Tg_tmp

#####################

    def run(self, i, j, n,
                  nsl, st, slprop, Sm, Sfc, Sr, Su_ini, Ss_ini, Rp_ini, D, Dl_ini, Ks, Ss_max, Ss_ratio,
                  TopAquif, HEADS, DRN, DRN_ini, cf,
...
#            D=0.05
        ...
        for t in range(int(nstp)):    # t: current time step
                ...
                if n == 0 and t == 0:
                    ...
                    for l in range(nsl):
                        if Dl[l] > 0.0:
                            ...
                            Su_ini[l] = Sm[l]*Dl[l]
                # fluxes
                Es_tmp, Ss_tmp, Ro_tmp, Rp_tmp, Eu_tmp, Tu_tmp, Su_tmp, Su_pc_tmp, R_tmp, Eg_tmp, Tg_tmp, HEADS_tmp, DRN_corr = self.flux(RFe_tot[t], PETveg[:,t], PE_tot[t], Su, Rp, Zr_elev, VEGarea, HEADS_tmp, Dltop, Dlbot, Dl, Dl_ini, nsl, Sm, Sfc, Sr, Ks, Ss_max, Ss_ratio, t, Su_ini, Rp_ini, DRN_tmp, DRN_ini, Ss_ini, E0[t], dtwt, st, i, j, n, kTu_min, kTu_n, dt, dti)
            ...
            results1[t,:] = [RF[t], PET_tot[t], PE_tot[t], RFe_tot[t], Ss[t], Ro[t], DRN_tmp/dt, Es[t], MB[t], INTER_tot[t], E0[t], Eg[t], Tg[t], R[t], Rn[t], dSs[t], ETg[t], ETu_tot[t], dSu_tot[t], HEADScorr[t]/1000.0]
        ...
        del RF, PET_tot, PE_tot, RFe_tot, Ss, Ro, DRN, Es, MB, INTER_tot, E0, Eg, Tg, R, Rn, dSs, ETg, ETu_tot, dSu_tot, HEADS_tmp, HEADScorr
