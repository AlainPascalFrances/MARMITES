...
#####################
    ...
    def flux(self, RFe, PT, PE, E0, Zr_elev, VEGarea, HEADS, TopSoilLay, BotSoilLay, Tl, nsl, Sm, Sfc, Sr, Ks, Ss_max, Ss_ratio, Su_ini, Rp_ini, Ss_ini, EXF, dtwt, st, i, j, n, kTu_min, kTu_n, facEg, dt, dti):
        ...
        if Ss_tmp == 0.0:
            if PE > 0.0:
                ...
                if dtwt <= dll:
                    ...
                    Eg_tmp = PE*(y0 + np.exp(-b*(dtwt-dll)))/facEg
                else:
                    Eg_tmp = 0.0
                dtwt *= 10.0
            else:
...
#####################
    ...
    def run(self, i, j, n,
        ...
        for t in range(int(nstp)):    # t: current time step
            ...
            # fluxes
            Es_tmp, Ss_tmp, Ro_tmp, Rp_tmp, Eu_tmp, Tu_tmp, Su_tmp, Su_pc_tmp, Eg_tmp, Tg_tmp, HEADS_tmp, dtwt_tmp, SAT_tmp, Rexf_tmp = self.flux(RFe_tot[t], PTveg[:,t], PE_tot[t], E0[t], Zr_elev, VEGarea, HEADS_tmp, TopSoilLay, BotSoilLay, Tl, nsl, Sm, Sfc, Sr, Ks, Ss_max, Ss_ratio, Su_ini, Rp_ini, Ss_ini, EXF[t], dtwt[t], st, i, j, n, kTu_min, kTu_n, facEg, dt, dti)
