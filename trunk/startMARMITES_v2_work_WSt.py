...
try:
    ...
    if plot_out == 1 and os.path.exists(h5_MM_fn):
        flxlbl = ['RF', 'RFe', 'INTER', 'SEEPAGE', 'dPOND', 'Ro', 'Es', 'R', 'Rn', 'Eg', 'Tg', 'ETg', 'ETu', 'dS']
        TSlst = []
        TS = 0
        while TS < sum(perlen):
            TSlst.append(TS)
        ...
        for i in flxlbl:
            ...
            Vmin = np.nanmin(V[0]) #float(np.floor(np.nanmin(V)))
            if Vmax<>0.0 or Vmin<>0.0:
                MMplot.plotLAYER(TS, ncol = ncol, nrow = nrow, nlay = nlay, nplot = 1, V = V,  cmap = plt.cm.Blues, CBlabel = (i + ' (mm/day)'), msg = 'no flux', plt_title = ('MM_'+i + '_average'), MM_ws = MM_ws, interval_type = 'arange', interval_diff = (Vmax - Vmin)/nrange, Vmax = Vmax, Vmin = Vmin, contours = ctrs, ntick = ntick)
            del V
            for TS in TSlst:
                V = [np.ma.masked_values(MM[TS,:,:,:], hnoflo, atol = 0.09)]
                V[0] = np.add.accumulate(V[0], axis = 0)[:,:,_nslmax-1]
