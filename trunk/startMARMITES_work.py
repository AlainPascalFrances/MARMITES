...
sys.path.append(r'E:\00code\MARMITES\trunk\MARMITESunsat')
import MARMITESunsat as MARMunsat
...
if obsCHECK==1:
    ...
    for o in range(len(obs.keys())):
        ...
        outFileExport[o].write('Date,RF,E0,PET,RFe,Inter,ETu,ETs,S,SUST,SUSTcount,Qs, Qscount,Rp,R,hSATFLOW,hMF,hmeas,Smeas,SSATpart, FLOODcount,MB\n')
...
# order of output is 0-RF, 1-PET, 2-RFe, 3-SUST, 4-Qs, 5-ETu, 6-S, 7-Rp, 8-countFLOOD, 9-ETs, 10-countSATpart, 11-MB, 12-R, 13-INTER, 14-countPOND, 15-countRunoff, 16-E0
        ...
        for j in range(ncol):
            ...
            if ibound[i,j,0]<>0:
                ...
                results_temp = MARMunsat.run(i, j,
                                             ...
                                             AqType  = AqType,
                                             hdry    = hdry)
                #RF, PET_tot, RFe_tot, SUST, Qs, ETu, S, Rp, countFLOOD, ETs, countSATpart, MB, INTER, countPOND, countRunoff, E0
                iRF=0
                iPET=1
                iRFe=2
                iSUST=3
                ...
                gridoutETs[i,j]     = results[i,j,iETs,:].sum()/divfact
                results_PERall[i,j,iETs] = results_PERall[i,j,iETs] + gridoutETs[i,j]
                ...
                outFileETs.write('%.6f'%(gridoutETs[i,j]) + ' ')
...
for i in range(nrow):
    for j in range(ncol):
        if ibound[i,j,0]<>0:
            ...
            outFileETs_PERall.write('%.6f'%(results_PERall[i,j,iETs]/nper) + ' ')
            ...
            outFileETs_PERall.write('%.6f'%(hnoflo) + ' ')
...
del gridoutRF, gridoutRFe, gridoutR, gridoutRp, gridoutETu, gridoutETs, gridoutS, results_temp, results
