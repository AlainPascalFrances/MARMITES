...
#capture interpreter output to be written in to a report file
    print '\nScreen output off, see run report at %s.' % report_fn
    s = StringIO.StringIO()
    sys.stdout = s
...
SP_d, nrow, ncol, delr, delc, nlay, perlen, nper, top, hnoflo, hdry, ibound, laytyp, h5_MF_fn, top_array, inputFileMF_fn, lenuni = ppMF.ppMF(MF_ws, rch_input = (h5_MM_fn, 'rch'), wel_input = (h5_MM_fn, 'ETg'))
...
while abs(h_diff[LOOP]) > convcrit:
    ...
    for n in range(nper):
        ...
        MM_rch = np.zeros([perlen[n],nrow,ncol], dtype=float)
        MM_ETg = np.zeros([perlen[n],nrow,ncol], dtype=float)
        h_MF = h5_MF['heads'][tstart:tend,:,:,0]
        drn_MF = h5_MF['cbc'][tstart:tend,iDRN,:,:,0]
        ncell = 0
        # loop into the grid
        for i in range(nrow):
            for j in range(ncol):
                ...
                if ibound[i,j,0]<>0.0:
                    ...
                    h_MF_tmp = h_MF[0:perlen[n],i,j]
                    drn_MF_tmp = -conv_fact*drn_MF[0:perlen[n],i,j]/(delr[j]*delc[i])
                    # for the while loop
                    ...
                    for l in range(nsl_tmp):
                        Si_tmp_array[i,j,l] = MM_S[perlen[n]-1,i,j,l,index_S.get('iSpc')]
                        Rpi_tmp_array[i,j,l] = MM_S[perlen[n]-1,i,j,l,index_S.get('iRp')]
                        PONDi_tmp_array[i,j] = MM[perlen[n]-1,i,j,index.get('iPOND')]
                    ...
                    for k in range(len(index_S)):
                        for l in range(nsl_tmp):
                            MM_S[:,i,j,l,k] = hnoflo
            h5_MM['MM'][tstart:tend,:,:,:] = MM[:,:,:,:]
            h5_MM['MM_S'][tstart:tend,:,:,:,:] = MM_S[:,:,:,:,:]
            h5_MM['rch'][tstart:tend,:,:] = MM_rch
            h5_MM['ETg'][tstart:tend,:,:] = MM_ETg
        del h_MF, drn_MF
...
del SP_d, nrow, ncol, delr, delc, nlay, perlen, nper, top, hnoflo, hdry, ibound, laytyp, h5_MF_fn, top_array, inputFileMF_fn, lenuni
