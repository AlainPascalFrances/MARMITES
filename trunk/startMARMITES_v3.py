...
print('\n##############\nMARMITES v0.3 started!\n%s\n##############' % mpl.dates.DateFormatter.format_data(fmt_DH,
                                                                                                           timestart))
...
try:
    ...
    MM_ws_out = os.path.join(MM_ws,
                             'out_%s_%s' % (mpl.dates.DateFormatter.format_data(fmt_DHshort, timestart), run_name))
    f = 1
    if os.path.exists(MM_ws_out):
        MM_ws_out1 = MM_ws_out
        while os.path.exists(MM_ws_out1):
            MM_ws_out1 = os.path.join(MM_ws, 'out_%s_%s_%s' % (
            mpl.dates.DateFormatter.format_data(fmt_DHshort, timestart), run_name, f))
            f += 1
    ...
    # max number of time series (in years) with trimesters minor grid
    l += 1
    maxYearsTickTrimester = int(inputFile[l].strip())
    #  max number of time series (in years) with semester minor grid
    l += 1
    maxYearsTickSemester = int(inputFile[l].strip())
    l += 1
    if maxYearsTickTrimester > maxYearsTickSemester:
        maxYearsTickTrimester = 5
        maxYearsTickSemester  = 10
    ...
    l += 1
    # MM-MF loop
    convcrit = Decimal(inputFile[l].strip()).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN)
    l += 1
    convcritmax = Decimal(inputFile[l].strip()).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN)
    l += 1
    ...
    l += 1
    rmseHEADSmax = Decimal(inputFile[l].strip()).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN)
    l += 1
    rmseSMmax = Decimal(inputFile[l].strip()).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN)
    l += 1
...
shutil.copy2(os.path.join(MM_ws, MM_ini_fn), os.path.join(MM_ws_out, '__%s%s' % (
mpl.dates.DateFormatter.format_data(fmt_DHshort, timestart), MM_ini_fn)))
# copy MF ini file into MM output folder
shutil.copy2(os.path.join(MM_ws, MF_ws, MF_ini_fn), os.path.join(MM_ws_out, '__%s%s' % (
mpl.dates.DateFormatter.format_data(fmt_DHshort, timestart), MF_ini_fn)))
# copy MMsurf ini file into MM output folder
shutil.copy2(os.path.join(MM_ws, MMsurf_ws, inputFile_PAR_fn), os.path.join(MM_ws_out, '__%s%s' % (
mpl.dates.DateFormatter.format_data(fmt_DHshort, timestart), inputFile_PAR_fn)))
# copy MM soil parameters file into MM output folder
shutil.copy2(os.path.join(MM_ws, SOILparam_fn), os.path.join(MM_ws_out, '__%s__%s' % (
mpl.dates.DateFormatter.format_data(fmt_DHshort, timestart), SOILparam_fn.split('\\')[-1])))
...
if verbose == 0:
    ...
    report_fn = os.path.join(MM_ws_out,
                             '__%s_MMMFrun_report.txt' % (mpl.dates.DateFormatter.format_data(fmt_DHshort, timestart)))
...
if MMsurf_yn > 0:
    ...
    if irr_yn == 1:
        outMMsurf_fn = startMMsurf.MMsurf(cUTIL, MMsurf_ws, inputFile_TS_fn, inputFile_PAR_fn, outputFILE_fn, MM_ws,
                                          outMMsurf_fn, MMsurf_plot, inputFile_TSirr_fn)
    else:
        outMMsurf_fn = startMMsurf.MMsurf(cUTIL, MMsurf_ws, inputFile_TS_fn, inputFile_PAR_fn, outputFILE_fn, MM_ws,
                                          outMMsurf_fn, MMsurf_plot)
...
try:
    ...
    line = inputFile[l].split()
    for v in range(NVEG):
        Zr.append(Decimal(float(line[v])).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN))
    l += 1
    line = inputFile[l].split()
    for v in range(NVEG):
        kTg_min.append(Decimal(float(line[v])).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN))
    l += 1
    line = inputFile[l].split()
    for v in range(NVEG):
        kTg_max.append(Decimal(float(line[v])).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN))
    l += 1
    line = inputFile[l].split()
    for v in range(NVEG):
        kT_f.append(Decimal(float(line[v])).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN))
    l += 1
    line = inputFile[l].split()
    for v in range(NVEG):
        kT_s.append(Decimal(1.0/float(line[v])).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN))
    if irr_yn == 1:
        l += 1
        ...
        for c in range(NCROP):
            Zr_c.append(Decimal(float(line[c])).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN))
        Zr_c = np.asarray(Zr_c)
        l += 1
        line = inputFile[l].split()
        for c in range(NCROP):
            kTg_min_c.append(Decimal(float(line[c])).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN))
        kTg_min_c = np.asarray(kTg_min_c)
        l += 1
        line = inputFile[l].split()
        for c in range(NCROP):
            kTg_max_c.append(Decimal(float(line[c])).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN))
        kTg_max_c = np.asarray(kTg_max_c)
        l += 1
        line = inputFile[l].split()
        for c in range(NCROP):
            kT_f_c.append(Decimal(float(line[c])).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN))
        kT_f_c = np.asarray(kT_f_c)
        l += 1
        line = inputFile[l].split()
        for c in range(NCROP):
            kT_s_c.append(Decimal(1.0/float(line[c])).quantize(Decimal('.00001'), rounding=ROUND_HALF_EVEN))
        kT_s_c = np.asarray(kT_s_c)
        l += 1
...
DATE = np.zeros(len(date), dtype=np.float32)
...
year_lst = []
HYindex = []
if mpl.dates.num2date(DATE[0]).month < iniMonthHydroYear or (
        mpl.dates.num2date(DATE[0]).month == iniMonthHydroYear and mpl.dates.num2date(DATE[0]).day == 1):
...
if sum(DATE[:] == mpl.dates.datestr2num('%d-%d-01' % (year_lst[0] + 1, iniMonthHydroYear))) == 0:
    ...
    while DATE[-1] >= mpl.dates.datestr2num('%d-%d-01' % (year_lst[y] + 1, iniMonthHydroYear)):
        ...
        if DATE[-1] >= mpl.dates.datestr2num('%d-%d-30' % (year_lst[y] + 2, iniMonth_prev)):
            ...
            HYindex.append(
                np.argmax(DATE[:] == mpl.dates.datestr2num('%d-%d-01' % (year_lst[y] + 1, iniMonthHydroYear))))
...
cMF = ppMF.clsMF(cUTIL, MM_ws, MM_ws_out, MF_ws, MF_ini_fn, xllcorner, yllcorner, numDays=numDays, stdout=stdout,
                 report=report)
...
if irr_yn == 1:
    ...
    if gridIRR.max() > NFIELD:
        cUTIL.ErrorExit('\nFATAL ERROR!\nThere is more fields in the asc file than in the MMsurf file!', stdout=stdout,
                        report=report)
...
# SOIL PARAMETERS
_nsl, _nam_soil, _st, _slprop, _Sm, _Sfc, _Sr, _S_ini, _Ks = cMF.cPROCESS.inputSoilParam(SOILparam_fn=SOILparam_fn,
                                                                                         NSOIL=NSOIL, stdout=None,
                                                                                         report=None)
_nslmax = max(_nsl)
...
cMF.top = np.ma.masked_values(cMF.elev, cMF.hnoflo, atol=0.09) - np.ma.masked_values(gridSOILthick, cMF.hnoflo,
                                                                                     atol=0.09)
cMF.botm = np.asarray(cMF.botm)
cMF.LandSurface = np.zeros(cMF.top.shape, dtype=np.float32)
for l in range(cMF.nlay):
    cMF.botm[l, :, :] = np.ma.masked_values(cMF.botm[l, :, :], cMF.hnoflo, atol=0.09) - np.ma.masked_values(
        gridSOILthick, cMF.hnoflo, atol=0.09)
...
# EXPORT INPUT MAPS
if plt_input == 1:
    ibound = np.asarray(np.abs(cMF.ibound))
    print('\n##############')
    print('Exporting input maps...')
    i_lbl = 1
    T = np.zeros((cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
    ...
    top_tmp = np.zeros((cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
    top_tmp[0, :, :] = cMF.top
    for l in range(1, cMF.nlay):
        top_tmp[l, :, :] = cMF.botm[l - 1, :, :]
    lst = [cMF.elev, top_tmp, cMF.botm, cMF.thick, np.asarray(cMF.strt), gridSOILthick, gridSsurfhmax, gridSsurfw,
           np.asarray(cMF.hk_actual), T, np.asarray(cMF.ss_actual), np.asarray(cMF.sy_actual),
           np.asarray(cMF.vka_actual)]
    lst_lbl = ['elev', 'top', 'botm', 'thick', 'strt', 'gridSOILthick', 'gridSsurfhmax', 'gridSsurfw', 'hk', 'T', 'Ss',
               'Sy', 'vka']
    lst_lblCB = ['Elev.', 'Aq. top - $top$', 'Aq. bot. - $botm$', 'Aq. thick.', 'Init. heads - $strt$', 'Soil thick.',
                 'Max. stream heigth', 'Stream width', 'Horizontal hydraulic cond. - $hk$', 'Transmissivity - $T$',
                 'Specific storage - $S_s$', 'Specific yield - $S_y$', 'Vertical hydraulic cond. - $vka$']
    if cMF.drn_yn == 1:
    ...
    for i, l in enumerate(lst):
        ...
        Vmax = []
        Vmin = []
        mask_tmp = np.zeros((cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
        for L in range(cMF.nlay):
            if l.shape == (cMF.nlay, cMF.nrow, cMF.ncol) or l.shape == (cMF.nrow, cMF.ncol):
                try:
                    V[0, L, :, :] = l[L, :, :]
                    mask_tmp[L, :, :] = mask[L]
                    Vmax.append(np.ma.max(
                        np.ma.masked_values(np.ma.masked_array(V[0, L, :, :], mask[L]), cMF.hnoflo, atol=0.09)))
                    Vmin.append(np.ma.min(
                        np.ma.masked_values(np.ma.masked_array(V[0, L, :, :], mask[L]), cMF.hnoflo, atol=0.09)))
                    nplot = cMF.nlay
                except:
                    V[0, L, :, :] = l
                    ...
                    Vmax.append(np.ma.max(
                        np.ma.masked_values(np.ma.masked_array(V[0, L, :, :], cMF.maskAllL), cMF.hnoflo, atol=0.09)))
                    Vmin.append(np.ma.min(
                        np.ma.masked_values(np.ma.masked_array(V[0, L, :, :], cMF.maskAllL), cMF.hnoflo, atol=0.09)))
                    nplot = 1
            elif l.shape != ():
                ...
                mask_tmp[L, :, :] = mask[L]
                Vmax.append(
                    np.ma.max(np.ma.masked_values(np.ma.masked_array(V[0, L, :, :], mask[L]), cMF.hnoflo, atol=0.09)))
                Vmin.append(
                    np.ma.min(np.ma.masked_values(np.ma.masked_array(V[0, L, :, :], mask[L]), cMF.hnoflo, atol=0.09)))
                nplot = cMF.nlay
            else:
                ...
                Vmin.append(np.ma.min(
                    np.ma.masked_values(np.ma.masked_array(V[0, L, :, :], cMF.maskAllL), cMF.hnoflo, atol=0.09)))
                nplot = 1
        if lst_lbl[i] == 'Ss' or lst_lbl[i] == 'hk' or lst_lbl[i] == 'T' or lst_lbl[i] == 'drn_cond' or lst_lbl[
            i] == 'ghb_cond' or lst_lbl[i] == 'vks':
            fmt = '%5.e'
        elif lst_lbl[i] == 'Sy' or lst_lbl[i] == 'thts' or lst_lbl[i] == 'thti' or lst_lbl[i] == 'thtr' or lst_lbl[
            i] == 'gridSOILthick' or lst_lbl[i] == 'gridSsurfhmax' or lst_lbl[i] == 'gridSsurfw':
            fmt = '%5.3f'
        else:
            fmt = '%5.1f'
...
#        try:
    ...
    Vmin = 0.0
    V = np.zeros((1, cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
    Vvegtot = np.zeros((1, cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
    for v in range(NVEG):
        V[0, 0, :, :] = gridVEGarea[v, :, :]
        ...
        try:
            MMplot.plotLAYER(days=[0], str_per=[0], Date='NA', JD='NA', ncol=cMF.ncol, nrow=cMF.nrow, nlay=cMF.nlay,
                         nplot=1, V=V, cmap=plt.cm.gist_rainbow_r, CBlabel=V_lblCB, msg='',
                         plt_title='IN_%03d_%s' % (i_lbl, V_lbl), MM_ws=MM_ws_out, interval_type=b'linspace',
                         interval_num=5, contours=False, Vmax=[Vmax], Vmin=[Vmin], ntick=ntick, fmt='%5.1f',
                         points=obs4map, mask=mask_tmp, hnoflo=cMF.hnoflo)
        except:
            print("ERROR plotting input map %s" % V_lbl)
        i_lbl += 1
    del V, V_lbl, V_lblCB
    ...
    for i, l in enumerate(lst):
        try:
            MMplot.plotLAYER(days=[0], str_per=[0], Date='NA', JD='NA', ncol=cMF.ncol, nrow=cMF.nrow, nlay=cMF.nlay,
                         nplot=1, V=l, cmap=plt.cm.gist_rainbow_r, CBlabel=lst_lblCB[i], msg='',
                         plt_title='IN_%03d_%s' % (i_lbl, lst_lbl[i]), MM_ws=MM_ws_out, interval_type=b'linspace',
                         interval_num=5, contours=False, Vmax=[Vmax], Vmin=[Vmin], ntick=ntick, fmt='%5.1f',
                         points=obs4map, mask=mask_tmp, hnoflo=cMF.hnoflo)
        except:
    ...
    lst = [cMF.ibound, gridSOIL, gridMETEO]
    ...
    for i, l in enumerate(lst):
        ...
        try:
            MMplot.plotLAYER(days=[0], str_per=[0], Date='NA', JD='NA', ncol=cMF.ncol, nrow=cMF.nrow, nlay=cMF.nlay,
                         nplot=nplot, V=V, cmap=plt.cm.gist_rainbow_r, CBlabel=lst_lblCB[i], msg='',
                         plt_title='IN_%03d_%s' % (i_lbl, lst_lbl[i]), MM_ws=MM_ws_out, interval_type=b'linspace',
                         interval_num=5, contours=False, Vmax=[Vmax], Vmin=[Vmin], ntick=ntick, fmt='%2.2f',
                         points=obs4map, mask=mask_tmp, hnoflo=cMF.hnoflo)
        except:
            print("ERROR plotting input map %s" % lst_lbl[i])
        i_lbl += 1
...
if MF_yn == 1:
    ...
    if verbose == 0:
        ...
        sys.stdout = report
    cMF.runMF(perc_MM=(h5_MM_fn, b'perc'), wel_MM=(h5_MM_fn, b'ETg'), verbose=verbose, chunks=chunks, numDays=numDays,
              stdout=stdout, report=report, obs=obs)
    timeendMF = mpl.dates.datestr2num(mpl.dates.datetime.datetime.today().isoformat())
...
if os.path.exists(cMF.h5_MF_fn):
    ...
    try:
        ...
        imfSTO = cbc_nam.index(b'STORAGE')
        imfFRF = cbc_nam.index(b'FLOW RIGHT FACE')
        imfFFF = cbc_nam.index(b'FLOW FRONT FACE')
        if cMF.nlay > 1:
            imfFLF = cbc_nam.index(b'FLOW LOWER FACE')
        if cMF.ghb_yn == 1:
            imfGHB = cbc_nam.index(b'HEAD DEP BOUNDS')
        if cMF.drn_yn == 1:
            imfDRN = cbc_nam.index(b'DRAINS')
        if cMF.wel_yn == 1:
            imfWEL = cbc_nam.index(b'WELLS')
        if len(cMF.ibound[cMF.ibound < 0]) > 0:
            imfCH = cbc_nam.index(b'CONSTANT HEAD')
        if cMF.uzf_yn == 1:
        ...
        cUTIL.ErrorExit('\nFATAL ERROR!\nInvalid MODFLOW HDF5 file. Run MARMITES and/or MODFLOW again.', stdout=stdout,
                        report=report)

# #############################
# 2nd phase : MM/MF loop #####
# #############################
h_diff_surf = np.asarray(None)
if MMsoil_yn != 0:
    durationMMsoil = 0.0
    h_pSP_average = 0
    h_pSP = 0
    ...
    h5_MM.create_dataset(name='iMM', data=np.asarray(list(index_MM.items())))
    if chunks == 1:
        h5_MM.create_dataset(name='MM', shape=(sum(cMF.perlen), cMF.nrow, cMF.ncol, len(index_MM)), dtype=np.float32,
                             chunks=(1, cMF.nrow, cMF.ncol, len(index_MM)), compression='gzip', compression_opts=5,
                             shuffle=True)
    else:
        h5_MM.create_dataset(name='MM', shape=(sum(cMF.perlen), cMF.nrow, cMF.ncol, len(index_MM)), dtype=np.float32)
    # arrays for fluxes in each soil layer
    h5_MM.create_dataset(name='iMM_S', data=np.asarray(list(index_MM_soil.items())))
    if chunks == 1:
        h5_MM.create_dataset(name='MM_S', shape=(sum(cMF.perlen), cMF.nrow, cMF.ncol, _nslmax, len(index_MM_soil)),
                             dtype=np.float32, chunks=(1, cMF.nrow, cMF.ncol, _nslmax, len(index_MM_soil)),
    ...
    if chunks == 1:
        h5_MM.create_dataset(name='perc', shape=(cMF.nper, cMF.nrow, cMF.ncol), dtype=np.float32,
                             chunks=(1, cMF.nrow, cMF.ncol), compression='gzip', compression_opts=5, shuffle=True)
        h5_MM.create_dataset(name='ETg', shape=(cMF.nper, cMF.nrow, cMF.ncol), dtype=np.float32,
                             chunks=(1, cMF.nrow, cMF.ncol), compression='gzip', compression_opts=5, shuffle=True)
    else:
        h5_MM.create_dataset(name='perc', shape=(cMF.nper, cMF.nrow, cMF.ncol), dtype=np.float32)
        h5_MM.create_dataset(name='ETg', shape=(cMF.nper, cMF.nrow, cMF.ncol), dtype=np.float32)
    ...
    while (abs(h_diff[LOOP]) > convcrit or abs(h_diff_all[LOOP]) > convcritmax) and LOOP <= ccnum:
        ...
        # SOIL PARAMETERS
        _nsl, _nam_soil, _st, _slprop, _Sm, _Sfc, _Sr, _S_ini, _Ks = cMF.cPROCESS.inputSoilParam(
            SOILparam_fn=SOILparam_fn, NSOIL=NSOIL, stdout=None, report=None)
        _nslmax = max(_nsl)
        for l in range(NSOIL):
            _slprop[l] = np.asarray(_slprop[l])
        if LOOP > 0:
        ...
        if h_MF_average == 0.0 or str(h_diff[LOOP]) == 'nan':
            ...
            msg_end_loop.append(
                'Successful convergence between MARMITES and MODFLOW!\n(Conv. criterion = %.4f and conv. crit. max. = %.4f)' % (
                convcrit, convcritmax))
            endloop += 1
        elif LOOP > ccnum:
            msg_end_loop.append(
        ...
        if not (endloop == 1 and MF_lastrun == 0):
            ...
            if verbose == 0:
                ...
                report = open(report_fn, 'a')
                sys.stdout = report
            cMF.runMF(perc_MM=(h5_MM_fn, 'perc'), wel_MM=(h5_MM_fn, 'ETg'), verbose=verbose, chunks=chunks,
                      numDays=numDays, stdout=stdout, report=report, obs=obs)
            try:
                h5_MF = h5py.File(cMF.h5_MF_fn, 'r')
            except:
    ...
    if MMsoil_yn > 0:
        ...
        fig = plt.figure()
        fig.suptitle(
            'Convergence loop plot between MM and MF based on heads differences.\nOrange: average heads differences for the whole model.\nGreen: maximun heads difference observed in the model (one cell)',
            fontsize=10)
        if LOOP > 0:
            ...
            plt.grid(True)
            plt.plot(LOOPlst[1:], h_diff[1:], linestyle='-', marker='o', markersize=5, c='orange',
                     markerfacecolor='orange', markeredgecolor='red')
            plt.plot(LOOPlst[1:], h_diff_all[1:], linestyle='-', marker='o', markersize=5, c='green',
        ...
        if LOOP > 1:
            ...
            plt.plot(LOOPlst[2:], h_diff[2:], linestyle='-', marker='o', markersize=5, c='orange',
                     markerfacecolor='orange', markeredgecolor='red')
            plt.plot(LOOPlst[2:], h_diff_all[2:], linestyle='-', marker='o', markersize=5, c='green',
                     markerfacecolor='green', markeredgecolor='blue')
            ...
            plt.plot(LOOPlst[2:], h_diff_log[2:], linestyle='-', marker='o', markersize=5, c='orange',
                     markerfacecolor='orange', markeredgecolor='red')
            plt.plot(LOOPlst[2:], h_diff_all_log[2:], linestyle='-', marker='o', markersize=5, c='green',
                     markerfacecolor='green', markeredgecolor='blue')
...
if MF_yn == 1 and isinstance(cMF.h5_MF_fn, str):
    ...
    if len(cMF.ibound[cMF.ibound < 0]) > 0:
        cMF.cPROCESS.procMF(cMF=cMF, h5_MF=h5_MF, ds_name='cbc', ds_name_new='CH_d', conv_fact=conv_fact, index=imfCH)
    cMF.cPROCESS.procMF(cMF=cMF, h5_MF=h5_MF, ds_name='heads', ds_name_new='heads_d', conv_fact=conv_fact)
    if cMF.uzf_yn == 1:
        cMF.cPROCESS.procMF(cMF=cMF, h5_MF=h5_MF, ds_name='cbc_uzf', ds_name_new='RCH_d', conv_fact=conv_fact,
                            index=imfRCH)
        cMF.cPROCESS.procMF(cMF=cMF, h5_MF=h5_MF, ds_name='cbc_uzf', ds_name_new='EXF_d', conv_fact=conv_fact,
                            index=imfEXF)
...
if MMsoil_yn != 0 and isinstance(h5_MM_fn, str):
    try:
        ...
        cUTIL.ErrorExit('\nFATAL ERROR!\nInvalid MARMITES HDF5 file. Run MARMITES and/or MODFLOW again.', stdout=stdout,
                        report=report)
...
if h_diff_surf.all() is not None and MMsoil_yn > 0:
    h_diff_n = None
    for n in range(cMF.nper):
    ...
    V = np.zeros((1, cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
    mask_tmp = np.zeros((cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
    if h_diff_n is not None:
        for L in range(cMF.nlay):
            V[0, L, :, :] = h_diff_surf[h_diff_n, L, :, :]
            mask_tmp[L, :, :] = mask[L]
            Vmax.append(
                np.ma.max(np.ma.masked_values(np.ma.masked_array(V[0, L, :, :], cMF.maskAllL), cMF.hnoflo, atol=0.09)))
            Vmin.append(
                np.ma.min(np.ma.masked_values(np.ma.masked_array(V[0, L, :, :], cMF.maskAllL), cMF.hnoflo, atol=0.09)))
...
if os.path.exists(cMF.h5_MF_fn):
    ...
    if RCHmax > 0.0:
        ...
        try:
            ...
            if plt_out_obs == 1:
                ...
                obs['RCHmax'] = {'x': x, 'y': y, 'i': i, 'j': j, 'lay': l, 'hi': 999, 'h0': 999, 'RC': 999, 'STO': 999, 'obs_h': None, 'obs_h_yn': 0, 'obs_SM': None, 'obs_sm_yn': 0, 'obs_Ro': None, 'obs_Ro_yn': 0}
            ...
            try:
                ...
                if plt_out_obs == 1:
                    ...
                    obs['ETgmax'] = {'x': x, 'y': y, 'i': i, 'j': j, 'lay': l, 'hi': 999, 'h0': 999, 'RC': 999, 'STO': 999,
                                     'obs_h': None, 'obs_h_yn': 0, 'obs_SM': None, 'obs_sm_yn': 0, 'obs_Ro': None, 'obs_Ro_yn': 0}
    ...
    if len(cMF.ibound[cMF.ibound < 0]) > 0:
        cbc_CH = h5_MF['CH_d']
        CHmax = np.ma.max(cbc_CH)
        cbcmax_d.append(CHmax)
        CHmin = np.ma.min(cbc_CH)
        cbcmin_d.append(CHmin)
        del cbc_CH
    # EXF
    cbc_EXF = h5_MF['EXF_d']
...
if os.path.exists(h5_MM_fn):
    try:
        ...
        cUTIL.ErrorExit('\nFATAL ERROR!\nInvalid MARMITES HDF5 file. Run MARMITES and/or MODFLOW again.', stdout=stdout,
                        report=report)
    # h
    headscorr_m = np.ma.masked_values(np.ma.masked_values(h5_MM['MM'][:, :, :, 19], cMF.hnoflo, atol=0.09), cMF.hdry,
                                      atol=1E+25)
...
if obs is not None:
    ...
    for o in list(obs.keys()):
        ...
        if os.path.exists(cMF.h5_MF_fn):
            ...
            hminMF_tmp = []
            for l in range(cMF.nlay):
                h_MF_m = np.ma.masked_values(np.ma.masked_values(h5_MF['heads_d'][:, l, :, :], cMF.hnoflo, atol=0.09), cMF.hdry,
                                         atol=1E+25)
                hmaxMF_tmp.append(np.ma.max(h_MF_m[:, i, j].flatten()))
                hminMF_tmp.append(np.ma.min(h_MF_m[:, i, j].flatten()))
                del h_MF_m
            h5_MF.close()
        else:
            hmaxMF_tmp = -9999.9
        ...
        if obs_h  is not None:
            ...
            hmaxMM = -9999.9
            hminMM = 9999.9
        hmax.append(max(np.ma.max(hmaxMF_tmp), hmaxMM))
        hmin.append(min(np.ma.min(hminMF_tmp), hminMM))
...
if plt_out_obs == 1 and os.path.exists(h5_MM_fn) and os.path.exists(cMF.h5_MF_fn):
    ...
    for z, (i, i_tex) in enumerate(zip(flx_lst, [r'${E_{soil}}$', r'${T_{soil}}$', r'$\theta$', r'${R_p}$', r'$Exf$',
                                                 r'${\Delta S_{soil}}$', r'${S_{soil}}}$', r'$SAT$',
                                                 r'${MB_{soil}}$'])):
    ...
    flxIndex_lst[b'iExf_1'] = count
    count += 1
    array_tmp = h5_MM['MM_S'][:, :, :, 0, index_MM_soil.get(b'iExf')]
    flx_tmp = np.ma.masked_values(array_tmp, cMF.hnoflo, atol=0.09)
    flxmax_d.append(np.ma.max(flx_tmp))
    flxmin_d.append(np.ma.min(flx_tmp))
    array_tmp1 = np.sum(np.ma.masked_values(array_tmp, cMF.hnoflo, atol=0.09), axis=1)
    ...
    del flx_tmp, array_tmp, array_tmp1
    if cMF.wel_yn == 1:
        array_tmp = h5_MM['MM'][:, :, :, index_MM.get(b'iEg')]
        flx_tmp = np.ma.masked_values(array_tmp, cMF.hnoflo, atol=0.09)
        flxmax_d.append(np.ma.max(flx_tmp))
        flxmin_d.append(np.ma.min(flx_tmp))
        Eg_tmp = np.zeros((sum(cMF.perlen)), dtype=np.float32)
        for L in range(cMF.nlay):
            ...
            for row in range(cMF.nrow):
                for t, col in enumerate(flx_tmp[:, row, :]):
                    try:
                        ...
                        tTgmin = t
                        if plt_out_obs == 1:
                            obs['PzTgmin'] = {'x': 999, 'y': 999, 'i': row, 'j': list(col).index(Tg_min), 'lay': 0,
                                              'hi': 999, 'h0': 999, 'RC': 999, 'STO': 999, 'obs_h': None, 'obs_h_yn': 0,
                                              'obs_SM': None, 'obs_sm_yn': 0, 'obs_Ro': None, 'obs_Ro_yn': 0}
                            obs_list.append('Tgm')
                        ...
                        pass
        Tg_tmp = np.zeros((sum(cMF.perlen)), dtype=np.float32)
        for L in range(cMF.nlay):
            mask_temp = mask_Lsup == (cMF.nlay - L)
            array_tmp1 = np.sum(array_tmp * mask_temp, axis=1)
            array_tmp2 = np.sum(np.ma.masked_values(array_tmp1, cMF.hnoflo, atol=0.09), axis=1)
    ...
    if os.path.exists(cMF.h5_MF_fn):
        ...
        try:
            ...
            cUTIL.ErrorExit('\nFATAL ERROR!\nInvalid MF HDF5 file. Run MARMITES and/or MODFLOW again.', stdout=stdout,
                            report=report)
        cbc_RCH = h5_MF['RCH_d']
        array_tmp2 = np.zeros((sum(cMF.perlen)), dtype=np.float32)
        rch_tot = 0
        ...
        flxIndex_lst[b'iRg'] = count
        count += 1
        flxLbl_lst.append(r'$\Delta S_{u}$')
        flxCatch_lst.append(flxCatch_lst[flxIndex_lst[b'iperc']] - rch_tot / sum(ncell_MM))
        flxIndex_lst[b'idSu'] = count
        count += 1
        del array_tmp, array_tmp1, rch_tmp, rch_tot, cbc_RCH
        for L in range(cMF.nlay):
            # ADD heads averaged
            if cMF.h_plt[L] == 1:
                flxLbl_lst.append(r'$h_{%s}$' % cMF.h_lbl[L]) #r'$h_{%d}$' % (L + 1))
                ...
                del array_tmp
                # ADD depth GWT
                flxLbl_lst.append(r'$d_{%s}$' % cMF.h_lbl[L])    #r'$d_{%d}$' % (L + 1))
                flxCatch_lst.append(flxCatch_lst[-1] - TopSoilAverage)
                i = b'id_%s' % cMF.h_lbl[L].encode('ASCII')  #b'id_%d' % (L + 1)
                flxIndex_lst[i] = count
                count += 1
                del array_tmp1
        for L in range(cMF.nlay):
            ...
            flxCatch_lst.append(np.zeros((sum(cMF.perlen)), dtype=np.int32))
            ...
            if cMF.nlay > 1:
                try:
                    ...
                    array_tmp2_L = np.sum(np.ma.masked_values(array_tmp1_L, cMF.hnoflo, atol=0.09), axis=1) / sum(
                        ncell_MM)
                    array_tmp2_Lm1 = np.sum(np.ma.masked_values(array_tmp1_Lm1, cMF.hnoflo, atol=0.09), axis=1) / sum(
                        ncell_MM)
            ...
            if len(cMF.ibound[cMF.ibound < 0]) > 0:
                cbc_CH = h5_MF['CH_d'][:, L, :, :]
                array_tmp1 = np.sum(np.ma.masked_values(cbc_CH, cMF.hnoflo, atol=0.09), axis=1)
                flxCatch_lst.append(
                    np.sum(np.ma.masked_values(array_tmp1, cMF.hnoflo, atol=0.09), axis=1) / sum(ncell_MM))
                del array_tmp1
...
if plt_out_obs == 1 and os.path.exists(h5_MM_fn) and os.path.exists(cMF.h5_MF_fn):
    ...
    try:
        ...
        cUTIL.ErrorExit('\nFATAL ERROR!\nInvalid MM or MF HDF5 file. Run MARMITES and/or MODFLOW again.', stdout=stdout,
                        report=report)
        valid = 1
    if valid == 0:
        ...
        for o_ref in obs_list:
            for o in list(obs.keys()):
                if o == o_ref:
                    ...
                    # SATFLOW
                    cbc_RCH = h5_MF['RCH_d']
                    h_satflow = MM_SATFLOW.runSATFLOW(cbc_RCH[:, l_highest, i, j], float(obs.get(o)['hi']),
                                                      float(obs.get(o)['h0']), float(obs.get(o)['RC']),
                                                      float(obs.get(o)['STO']))
                    ...
                    if irr_yn == 0:
                        index_veg = np.ones((NVEG, sum(cMF.perlen)), dtype=np.float32)
                        for v in range(NVEG):
                    ...
                    flx_lst = []
                    for e in sorted(index_MM_soil, key=index_MM_soil.get): flx_lst.append(e)
                    for z, (ii, ii_tex) in enumerate(zip(flx_lst, ['E_{soil}', 'T_{soil}', '\\thetaWRONG', 'R_p', 'Exf',
                                                                   '\Delta S_{soil}WRONG', 'S_{soil}WRONG', 'SATWRONG',
                                                                   'MB_{soil}'])):
                        flxLbl_lst.append(r'$%s$' % ii_tex)
                        flxObs_lst.append((np.sum(
                            np.ma.masked_values(MM_S[:, :, index_MM_soil.get(ii)], cMF.hnoflo, atol=0.09), axis=1)))
                        flxIndex_lst[ii] = count
                        count += 1
                    ...
                    for z, (ii, ii_tex) in enumerate(zip(flx_lst,[r'E_{soil}', 'T_{soil}','\\theta', 'R_{soil}', 'Exf','\Delta \\theta', 'S_{soil}', 'SAT', 'MB_{soil}'])):
                        for l in range(nsl):
                            ...
                            flxIndex_lst[b'%s_%d' % (ii, l + 1)] = count
                            count += 1
                    del flx_lst
                    # ET fluxes
                    if cMF.wel_yn == 1:
                    ...
                    flxIndex_lst[b'iperc_d'] = count
                    count += 1
                    flxLbl_lst.append(r'$\Delta S_{u}$')
                    flxObs_lst.append(flxObs_lst[flxIndex_lst[b'iperc_d']] - flxObs_lst[flxIndex_lst[b'iRg']])
                    flxIndex_lst[b'idSu'] = count
                    count += 1
                    for L in range(cMF.nlay):
                        # GW STO
                        ...
                        del cbc_STO
                        flxLbl_lst.append(r'$\Delta S_{g,%d}$' % (L + 1))
                        flxIndex_lst[b'idSg_%d' % (L + 1)] = count
                        count += 1
                        # GW Rg
                        cbc_R = h5_MF['RCH_d']
                        ...
                        del cbc_R
                        flxLbl_lst.append(r'$Rg_{%d}$' % (L + 1))
                        flxIndex_lst[b'iRg_%d' % (L + 1)] = count
                        count += 1
                        # GW FRF
                        try:
                        ...
                        flxLbl_lst.append('$FRF_{%d}$' % (L + 1))
                        flxIndex_lst[b'iFRF_%d' % (L + 1)] = count
                        count += 1
                        # GW FFF
                        try:
                        ...
                        flxLbl_lst.append('$FFF_{%d}$' % (L + 1))
                        flxIndex_lst[b'iFFF_%d' % (L + 1)] = count
                        count += 1
                        # GW FLF
                        if cMF.nlay > 1:
                            try:
                            ...
                            flxLbl_lst.append('$FLF_{%d}$' % (L + 1))
                            flxIndex_lst[b'iFLF_%d' % (L + 1)] = count
                            count += 1
                        # EXF
                        flxObs_lst.append(h5_MF['EXF_d'][:, L, i, j])
                        flxLbl_lst.append('$Exf_{g,%d}$' % (L + 1))
                        flxIndex_lst[b'iEXFg_%d' % (L + 1)] = count
                        count += 1
                        # WEL
                        if cMF.wel_yn == 1:
                            flxObs_lst.append(h5_MF['WEL_d'][:, L, i, j])
                            flxLbl_lst.append('$WEL_{%d}$' % (L + 1))
                            flxIndex_lst[b'iWEL_%d' % (L + 1)] = count
                            count += 1
                            # DRN
                        if cMF.drn_yn == 1:
                            flxObs_lst.append(h5_MF['DRN_d'][:, L, i, j])
                            flxLbl_lst.append('$DRN_{%d}$' % (L + 1))
                            flxIndex_lst[b'iDRN_%d' % (L + 1)] = count
                            count += 1
                            # GHB
                        if cMF.ghb_yn == 1:
                            flxObs_lst.append(h5_MF['GHB_d'][:, L, i, j])
                            flxLbl_lst.append('$GHB_{%d}$' % (L + 1))
                            flxIndex_lst[b'iGHB_%d' % (L + 1)] = count
                            count += 1
                        # CH
                        if len(cMF.ibound[cMF.ibound < 0]) > 0:
                            flxObs_lst.append(h5_MF['CH_d'][:, L, i, j])
                            flxLbl_lst.append('$CH_{%d}$' % (L + 1))
                            flxIndex_lst[b'iCH_%d' % (L + 1)] = count
                            count += 1
                    flxObs_lst.append(SP_d)
                    #flxIndex_lst.append(count)
                    flxLbl_lst.append(r'SP_d')
                    count += 1
                    ...
                    # remove decimal from _Sm, _Sfc, _Sr e _Ks for pretty print
                    for l_ in range(len(_Sm)):
                        for l__ in range(len(_Sm[l_])):
                            _Sm[l_][l__] = float(_Sm[l_][l__])
                            _Sfc[l_][l__] = float(_Sfc[l_][l__])
                            _Sr[l_][l__] = float(_Sr[l_][l__])
                            _Ks[l_][l__] = float(_Ks[l_][l__])
                    plt_title = 'i = %d, j = %d, l_obs = %d, l_highest = %d, x = %.2f, y = %.2f, elev. = %.2f, %s\nSm = %s, Sfc = %s, Sr = %s, Ks = %s, thick. = %.3f %s\nfrac. area veg. = %s (veg. name = %s)\nfrac. area veg. tot. = %.1f, frac. area soil = %.1f' % (
                    ...
                    try:
                        MMplot.plotTIMESERIES(cMF, i, j, flxObs_lst, flxLbl_lst, flxIndex_lst,
                                              ...
                                              clr_lst,
                                              max(hmax), min(hmin),
                                              o, l_obs, nsl, cMF.elev[i, j],
                                              iniMonthHydroYear, date_ini=DATE[HYindex[1]],
                                              date_end=DATE[HYindex[-2]],
                                              maxYearsTickTrimester = maxYearsTickTrimester, maxYearsTickSemester = maxYearsTickSemester)
                        print('TS plot done!')
                    except:
                       print('TS plot error!')
                    ...
                    if WBsankey_yn == 1:
                        try:
                            ...
                            MMplot.plotWBsankey(MM_ws_out, cMF.inputDate, flxObs_lst, flxIndex_lst,
                                                ...
                                                fntitle='0%s' % o, ibound4Sankey=ibound4Sankey,
                                                stdout=stdout, report=report)
                            print('WB Sankey plot done!')
                        except:
                            print('WB Sankey plot error!')
                    ...
                    if obs_S is not None:
                        ...
                        obs_SM_tmp = []
                    MM_S = h5_MM['MM_S'][HYindex[1]:HYindex[-2], i, j, 0:nsl, index_MM_soil.get(b'iSsoil_pc_s')]
                    if obs_h is not None:
                    ...
                    rmseHEADS_tmp, rmseHEADSc_tmp, rmseSM_tmp, rsrHEADS_tmp, rsrHEADSc_tmp, rsrSM_tmp, nseHEADS_tmp, nseHEADSc_tmp, nseSM_tmp, rHEADS_tmp, rHEADSc_tmp, rSM_tmp = cMF.cPROCESS.compCalibCritObs(
                        ...
                        MM[HYindex[1]:HYindex[-2], index_MM.get(b'ihcorr')])
        ...
        if len(obslstHEADS) > 0 or len(obslstSM) > 0:
            ...
            try:
                for cc, (rmse, rsr, nse, r, obslst, msg) in enumerate(
                        zip([rmseSM, rmseHEADS], [rsrSM, rsrHEADS], [nseSM, nseHEADS], [rSM, rHEADS],
                            [obslstSM, obslstHEADS], ['SM (all layers): %.1f %% /', 'h: %.2f m /'])):
...
if plt_out == 1 and os.path.exists(h5_MM_fn) and os.path.exists(cMF.h5_MF_fn):
    ...
    # ############################################
    mask_tmp = np.zeros((cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
    maskAllL_tmp = np.zeros((cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
    for L in range(cMF.nlay):
        mask_tmp[L, :, :] = mask[L]
        maskAllL_tmp[L, :, :] = cMF.maskAllL
    if plt_freq > 0:
        # plot heads [m]
        V = np.zeros((len(days_lst), cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
        Vmax = np.zeros((len(days_lst)), dtype=np.float32)
        Vmin = np.zeros((len(days_lst)), dtype=np.float32)
        Vmax1 = np.zeros((len(days_lst)), dtype=np.float32)
        ...
        try:
            MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                         ...
                         CBlabel='$h$ (m)', msg='DRY', plt_title='OUT_MF_HEADS',
        ...
        try:
            MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                         nlay=cMF.nlay, nplot=cMF.nlay, V=V, cmap=plt.cm.Blues,
                         CBlabel='$d$ (m)', msg='DRY', plt_title='OUT_MF_GWTD',
                         MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, contours=ctrsMF, Vmax=Vmax, Vmin=Vmin,
                         ntick=ntick, points=obs4map, mask=mask_tmp, hnoflo=cMF.hnoflo, animation=animation, cMF=cMF)
        except:
            print("ERROR plotting map of dgw")

        # plot heads corrigidas [m]
        headscorr_m = np.zeros((len(days_lst), cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
        for i, t in enumerate(days_lst):
            headscorr_m[i, 0, :, :] = np.ma.masked_values(
                np.ma.masked_values(h5_MM['MM'][t, :, :, index_MM.get(b'ihcorr')], cMF.hnoflo, atol=0.09), cMF.hdry,
                atol=1E+25)
            Vmin[i] = hcorrmin
            Vmax[i] = hcorrmax
        try:
            MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                         ...
                         CBlabel='$h$ (m)', msg='DRY', plt_title='OUT_MF_HEADScorr',
                         MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, contours=ctrsMF, Vmax=Vmax, Vmin=Vmin,
                         ntick=ntick, points=obs4map, mask=mask_tmp, hnoflo=cMF.hnoflo, animation=animation, cMF=cMF)
        except:
            print("ERROR plotting map of heads corrected")

        # plot GWTD correct [m]
        GWTDcorr = np.zeros((len(days_lst), cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
        for i, t in enumerate(days_lst):
        ...
        try:
            MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                         nlay=cMF.nlay, nplot=1, V=GWTDcorr, cmap=plt.cm.Blues,
                         CBlabel='$d$ (m)', msg='DRY', plt_title='OUT_MF_GWTDcorr',
                         MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, contours=ctrsMF, Vmax=Vmax, Vmin=Vmin,
                         ntick=ntick, points=obs4map, mask=mask_tmp, hnoflo=cMF.hnoflo, animation=animation, cMF=cMF)
        except:
            print("ERROR plotting map of dgw corrected")

        # plot GW GROSS RCH [mm]
        Rg = np.zeros((len(days_lst), cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
        for i, t in enumerate(days_lst):
            for L in range(cMF.nlay):
        ...
        try:
            MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                         nlay=cMF.nlay, nplot=cMF.nlay, V=Rg, cmap=plt.cm.Blues,
                         CBlabel='$Rg$ (mm.day$^{-1}$)', msg='- no flux',
                         plt_title='OUT_MF_Rg', MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin,
                         contours=ctrsMF, Vmax=Vmax, ntick=ntick, points=obs4map, hnoflo=cMF.hnoflo, mask=mask_tmp,
                         animation=animation, cMF=cMF)
            MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                         nlay=cMF.nlay, nplot=cMF.nlay, V=Rg, cmap=plt.cm.Blues,
                         CBlabel='$Rg$ (mm.day$^{-1}$)', msg='- no flux',
                         plt_title='OUT_MF_Rg1', MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin1,
                         contours=ctrsMF, Vmax=Vmax1, ntick=ntick, points=obs4map, hnoflo=cMF.hnoflo, mask=mask_tmp,
                         animation=animation, cMF=cMF)
        except:
            print("ERROR plotting map of Rg")

        # plot GW EFFECTIVE RCH [mm]
        Re = np.zeros((len(days_lst), cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
        for i, t in enumerate(days_lst):
            for L in range(cMF.nlay):
        ...
        try:
            MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                         ...
                         CBlabel='$Re$ (mm.day$^{-1}$)', msg='- no flux',
                         plt_title='OUT_MF_Re', MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin,
                         contours=ctrsMF, Vmax=Vmax, ntick=ntick, points=obs4map, hnoflo=cMF.hnoflo, mask=mask_tmp,
                         animation=animation, cMF=cMF)
            MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                         nlay=cMF.nlay, nplot=cMF.nlay, V=Re, cmap=plt.cm.Blues_r,
                         CBlabel='$Re$ (mm.day$^{-1}$)', msg='- no flux',
                         plt_title='OUT_MF_Re1', MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin1,
                         contours=ctrsMF, Vmax=Vmax1, ntick=ntick, points=obs4map, hnoflo=cMF.hnoflo, mask=mask_tmp,
                         animation=animation, cMF=cMF)
        except:
            print("ERROR plotting map of Re")

        # plot GW NET RCH [mm]
        if cMF.wel_yn == 1:
            Rn = np.zeros((len(days_lst), cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
            for i, t in enumerate(days_lst):
                for L in range(cMF.nlay):
            ...
            try:
                MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                             ...
                             CBlabel='$Rn$ (mm.day$^{-1}$)', msg='- no flux',
                             plt_title='OUT_MF_Rn', MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin,
                             contours=ctrsMF, Vmax=Vmax, ntick=ntick, points=obs4map, hnoflo=cMF.hnoflo, mask=mask_tmp,
                             animation=animation, cMF=cMF)
                MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                             nlay=cMF.nlay, nplot=cMF.nlay, V=Rn, cmap=plt.cm.Blues,
                             CBlabel='$Rn$ (mm.day$^{-1}$)', msg='- no flux',
                             plt_title='OUT_MF_Rn1', MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin1,
                             contours=ctrsMF, Vmax=Vmax1, ntick=ntick, points=obs4map, hnoflo=cMF.hnoflo, mask=mask_tmp,
                             animation=animation, cMF=cMF)
            except:
                print("ERROR plotting map of Rn")
            del Rn
        del Rg, Re
        ...
        if cMF.drn_yn == 1:
            ...
            try:
                MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                             ...
                             plt_title='OUT_MF_DRN', MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin,
                             contours=ctrsMF, Vmax=Vmax, ntick=ntick, points=obs4map, mask=mask_tmp, hnoflo=cMF.hnoflo,
                             animation=animation, cMF=cMF)
                MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                             nlay=cMF.nlay, nplot=cMF.nlay, V=V, cmap=plt.cm.Blues,
                             CBlabel='$DRN$ (mm.day$^{-1}$)', msg='- no drainage',
                             plt_title='OUT_MF_DRN1', MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin1,
                             contours=ctrsMF, Vmax=Vmax1, ntick=ntick, points=obs4map, mask=mask_tmp, hnoflo=cMF.hnoflo,
                             animation=animation, cMF=cMF)
            except:
                print("ERROR plotting map of DRN")
        ...
        if cMF.ghb_yn == 1:
            ...
            try:
                MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                             nlay=cMF.nlay, nplot=cMF.nlay, V=V, cmap=plt.cm.Blues,
                             CBlabel='$GHB$ (mm.day$^{-1}$)', msg='- no flux', plt_title='OUT_MF_GHB',
                             MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin, contours=ctrsMF,
                             Vmax=Vmax, ntick=ntick, points=obs4map, hnoflo=cMF.hnoflo, mask=mask_tmp, animation=animation,
                             cMF=cMF)
                MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                             nlay=cMF.nlay, nplot=cMF.nlay, V=V, cmap=plt.cm.Blues,
                             CBlabel='$GHB$ (mm.day$^{-1}$)', msg='- no flux', plt_title='OUT_MF_GHB1',
                             MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin1, contours=ctrsMF,
                             Vmax=Vmax1, ntick=ntick, points=obs4map, hnoflo=cMF.hnoflo, mask=mask_tmp, animation=animation,
                             cMF=cMF)
            except:
                print("ERROR plotting map of GHB")
        ...
        if len(cMF.ibound[cMF.ibound < 0]) > 0:
            ...
            for i, t in enumerate(days_lst):
                for L in range(cMF.nlay):
                    V[i, L, :, :] = np.ma.masked_array(cbc_CH[t, L, :, :], mask[L]) * (-1.0)
                Vmin[i] = CHmin
                Vmax[i] = CHmax
                Vmin1[i] = np.ma.min(V[i, :, :, :])
                Vmax1[i] = np.ma.max(V[i, :, :, :])
            try:
                MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                             nlay=cMF.nlay, nplot=cMF.nlay, V=V, cmap=plt.cm.Blues,
                             CBlabel='$CH$ (mm.day$^{-1}$)', msg='- no flux', plt_title='OUT_MF_CH',
                             MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin, contours=ctrsMF,
                             Vmax=Vmax, ntick=ntick, points=obs4map, hnoflo=cMF.hnoflo, mask=mask_tmp, animation=animation,
                             cMF=cMF)
                MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                             nlay=cMF.nlay, nplot=cMF.nlay, V=V, cmap=plt.cm.Blues,
                             CBlabel='$CH$ (mm.day$^{-1}$)', msg='- no flux', plt_title='OUT_MF_CH1',
                             MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin1, contours=ctrsMF,
                             Vmax=Vmax1, ntick=ntick, points=obs4map, hnoflo=cMF.hnoflo, mask=mask_tmp, animation=animation,
                             cMF=cMF)
            except:
                print("ERROR plotting map of CH")
        ...
        try:
            MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                         nlay=cMF.nlay, nplot=cMF.nlay, V=V, cmap=plt.cm.Reds,
                         CBlabel='$Exf_g$ (mm.day$^{-1}$)', msg='- no exfiltration',
                         plt_title='OUT_MF_EXF', MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin,
                         contours=ctrsMF, Vmax=Vmax, ntick=ntick, points=obs4map, mask=mask_tmp, hnoflo=cMF.hnoflo,
                         animation=animation, cMF=cMF)
            MMplot.plotLAYER(days=days_lst, str_per=sp_lst, Date=Date_lst, JD=JD_lst, ncol=cMF.ncol, nrow=cMF.nrow,
                         nlay=cMF.nlay, nplot=cMF.nlay, V=V, cmap=plt.cm.Reds,
                         CBlabel='$Exf_g$ (mm.day$^{-1}$)', msg='- no exfiltration',
                         plt_title='OUT_MF_EXF1', MM_ws=MM_ws_out, interval_type=b'linspace', interval_num=5, Vmin=Vmin1,
                         contours=ctrsMF, Vmax=Vmax1, ntick=ntick, points=obs4map, mask=mask_tmp, hnoflo=cMF.hnoflo,
                         animation=animation, cMF=cMF)
        except:
    ...
    for i, int_typ in enumerate([b'percentile', b'linspace']):
        try:
            MMplot.plotLAYER(days=['NA'], str_per=['NA'], Date=['NA'], JD=['NA'], ncol=cMF.ncol, nrow=cMF.nrow,
                         nlay=cMF.nlay, nplot=cMF.nlay, V=V, cmap=plt.cm.Blues,
                         CBlabel='$d$ (m)', msg='DRY',
                         plt_title=b'OUT_average_MF_GWTD%s' % int_typ, MM_ws=MM_ws_out, interval_type=int_typ,
                         interval_num=5, contours=ctrsMF, Vmax=[GWTDmax], Vmin=[GWTDmin], ntick=ntick, points=obs4map,
                         ptslbl=0, mask=mask_tmp, hnoflo=cMF.hnoflo, cMF=cMF)
        except:
            print("ERROR plotting map of dgw average")

    # plot heads corrigidas average [m]
    headscorr_m = np.zeros((1, cMF.nlay, cMF.nrow, cMF.ncol), dtype=np.float32)
    headscorr_m[0, 0, :, :] = np.ma.masked_values(np.ma.masked_values(
    ...
    for i, int_typ in enumerate([b'percentile', b'linspace']):
        try:
            MMplot.plotLAYER(days=['NA'], str_per=['NA'], Date=['NA'], JD=['NA'], ncol=cMF.ncol, nrow=cMF.nrow,
                         nlay=cMF.nlay, nplot=1, V=GWTDcorr, cmap=plt.cm.Blues,
                         CBlabel='$d$ (m)', msg='DRY',
                         plt_title=b'OUT_average_MF_GWTDcorr%s' % int_typ, MM_ws=MM_ws_out, interval_type=int_typ,
                         interval_num=5, contours=ctrsMF, Vmax=[GWTDcorrmax], Vmin=[GWTDcorrmin], ntick=ntick,
                         points=obs4map, ptslbl=0, mask=maskAllL_tmp, hnoflo=cMF.hnoflo, cMF=cMF)
        except:
            print("ERROR plotting map of dgw corrected average")
    ...
    for i, int_typ in enumerate([b'linspace']):
        try:
            MMplot.plotLAYER(days=['NA'], str_per=['NA'], Date=['NA'], JD=['NA'], ncol=cMF.ncol, nrow=cMF.nrow,
                         nlay=cMF.nlay, nplot=cMF.nlay, V=Rg, cmap=plt.cm.Blues,
                         CBlabel='$Rg$ (mm.day$^{-1}$)', msg='- no flux',
                         plt_title=b'OUT_average_MF_Rg1%s' % int_typ, MM_ws=MM_ws_out, interval_type=int_typ,
    ...
    for i, int_typ in enumerate([b'percentile', b'linspace']):
        try:
            MMplot.plotLAYER(days=['NA'], str_per=['NA'], Date=['NA'], JD=['NA'], ncol=cMF.ncol, nrow=cMF.nrow,
                         nlay=cMF.nlay, nplot=cMF.nlay, V=Re, cmap=plt.cm.Blues,
                         CBlabel='$Re$ (mm.day$^{-1}$)', msg='- no flux',
                         plt_title=b'OUT_average_MF_Re1%s' % int_typ, MM_ws=MM_ws_out, interval_type=int_typ,
                         interval_num=5, Vmax=[np.ma.max(Re)], Vmin=[np.ma.min(Re)], contours=ctrsMF, ntick=ntick,
                         points=obs4map, ptslbl=0, mask=mask_tmp, hnoflo=cMF.hnoflo, cMF=cMF)
        except:
            print("ERROR plotting map of Re average")
    Vmin_tmp1 = np.min(Re)
    Vmax_tmp1 = np.max(Re)
        ...
        for i, int_typ in enumerate([b'percentile', b'linspace']):
            try:
                MMplot.plotLAYER(days=['NA'], str_per=['NA'], Date=['NA'], JD=['NA'], ncol=cMF.ncol, nrow=cMF.nrow,
                             nlay=cMF.nlay, nplot=cMF.nlay, V=Rn, cmap=plt.cm.Blues,
                             CBlabel='$Rn$ (mm.day$^{-1}$)', msg='- no flux',
                             plt_title=b'OUT_average_MF_Rn1%s' % int_typ, MM_ws=MM_ws_out, interval_type=int_typ,
        ...
        for i, int_typ in enumerate([b'linspace']):
            try:
                MMplot.plotLAYER(days=['NA'], str_per=['NA'], Date=['NA'], JD=['NA'], ncol=cMF.ncol, nrow=cMF.nrow,
                             nlay=cMF.nlay, nplot=cMF.nlay, V=V, cmap=plt.cm.Blues,
                             CBlabel='$DRN$ (mm.day$^{-1}$)', msg='- no drainage',
                             plt_title=b'OUT_average_MF_DRN1%s' % int_typ, MM_ws=MM_ws_out, interval_type=int_typ,
                             interval_num=5, Vmax=[Vmax_tmp1], Vmin=[Vmin_tmp1], contours=ctrsMF, ntick=ntick,
                             points=obs4map, ptslbl=0, mask=mask_tmp, hnoflo=cMF.hnoflo, cMF=cMF)
            except:
                print("ERROR plotting map of DRN average")
        ...
        for i, int_typ in enumerate([b'linspace']):
            try:
                MMplot.plotLAYER(days=['NA'], str_per=['NA'], Date=['NA'], JD=['NA'], ncol=cMF.ncol, nrow=cMF.nrow,
                             nlay=cMF.nlay, nplot=cMF.nlay, V=V, cmap=plt.cm.Blues,
                             CBlabel='$GHB$ (mm.day$^{-1}$)', msg='- no flux',
                             plt_title=b'OUT_average_MF_GHB1%s' % int_typ, MM_ws=MM_ws_out, interval_type=int_typ,
                             interval_num=5, Vmax=[Vmax_tmp1], Vmin=[Vmin_tmp1], contours=ctrsMF, ntick=ntick,
                             points=obs4map, ptslbl=0, mask=mask_tmp, hnoflo=cMF.hnoflo, cMF=cMF)
            except:
                print("ERROR plotting map of GHB average")
    ...
    if len(cMF.ibound[cMF.ibound < 0]) > 0:
        ...
        for L in range(cMF.nlay):
            V[0, L, :, :] = np.ma.masked_array(
                np.sum(cbc_CH[HYindex[1]:HYindex[-2], L, :, :], axis=0) / (HYindex[-2] - HYindex[1] + 1) * (-1.0),
                mask[L])
        for i, int_typ in enumerate([b'percentile', b'linspace']):
            try:
                MMplot.plotLAYER(days=['NA'], str_per=['NA'], Date=['NA'], JD=['NA'], ncol=cMF.ncol, nrow=cMF.nrow,
        ...
        for i, int_typ in enumerate([b'linspace']):
            try:
                MMplot.plotLAYER(days=['NA'], str_per=['NA'], Date=['NA'], JD=['NA'], ncol=cMF.ncol, nrow=cMF.nrow,
                             nlay=cMF.nlay, nplot=cMF.nlay, V=V, cmap=plt.cm.Blues,
                             CBlabel='$CH$ (mm.day$^{-1}$)', msg='- no flux',
                             plt_title=b'OUT_average_MF_CH1%s' % int_typ, MM_ws=MM_ws_out, interval_type=int_typ,
                             interval_num=5, Vmax=[Vmax_tmp1], Vmin=[Vmin_tmp1], contours=ctrsMF, ntick=ntick,
                             points=obs4map, ptslbl=0, mask=mask_tmp, hnoflo=cMF.hnoflo, cMF=cMF)
            except:
                print("ERROR plotting map of CH average")
    ...
    if os.path.exists(h5_MM_fn):
        ...
        for z, (i, i_lbl, i_cmap) in enumerate(zip(flx_lst, flxLbl_lst, cmap_lst)):
            ...
            V = np.zeros((1, 1, cMF.nrow, cMF.ncol), dtype=np.float32)
            V[0, 0, :, :] = np.sum(np.ma.masked_values(MM[HYindex[1]:HYindex[-2], :, :], cMF.hnoflo, atol=0.09),
                                   axis=0) / (HYindex[-2] - HYindex[1] + 1)
            ...
            MMplot.plotLAYER(days=['NA'], str_per=['NA'], Date=['NA'], JD=['NA'], ncol=cMF.ncol, nrow=cMF.nrow,
                             ...
                             msg='no flux', plt_title=b'OUT_average_MM_' + i, MM_ws=MM_ws_out,
            ...
            # ############################################
            if plt_freq > 0:
                V = np.zeros((len(days_lst), 1, cMF.nrow, cMF.ncol), dtype=np.float32)
                Vmax = np.zeros((len(days_lst)), dtype=np.float32)
                Vmin = np.zeros((len(days_lst)), dtype=np.float32)
                Vmax1 = np.zeros((len(days_lst)), dtype=np.float32)
                Vmin1 = np.zeros((len(days_lst)), dtype=np.float32)
        ...
        for z, (i, i_lbl) in enumerate(zip(flx_lst, flxLbl_lst)):
            ...
            if plt_freq > 0:
                ...
                for ii, t in enumerate(days_lst):
                    ...
                    Vmin1[ii] = np.ma.min(
                        np.ma.masked_values(np.ma.masked_array(V[ii, 0, :, :], cMF.maskAllL), cMF.hnoflo, atol=0.09))
                    Vmax1[ii] = np.ma.max(
                        np.ma.masked_values(np.ma.masked_array(V[ii, 0, :, :], cMF.maskAllL), cMF.hnoflo, atol=0.09))
                for ii, t in enumerate(days_lst):
                    Vmax[ii] = np.ma.max(Vmax1)
        ...
        V[0, 0, :, :] = np.sum(np.ma.masked_values(MM[HYindex[1]:HYindex[-2], :, :], cMF.hnoflo, atol=0.09), axis=0) / (
                    HYindex[-2] - HYindex[1] + 1)
        Vmax = [np.ma.max(V)]
        Vmin = [np.ma.min(V)]
        try:
...
print('\n##############\nMARMITES executed successfully!\n%s' % mpl.dates.DateFormatter.format_data(fmt_DH,
...
if verbose == 0:
    sys.stdout = stdout
    report.close()
    print('##############\nMARMITES terminated normally!\n%s\n##############' % mpl.dates.DateFormatter.format_data(
        fmt_DH, mpl.dates.datestr2num(mpl.dates.datetime.datetime.today().isoformat())))
