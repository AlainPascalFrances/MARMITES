...
h5_rch_fn = os.path.join(MF_ws,'h5_MM4MF.h5')
SP_d, nrow, ncol, delr, delc, nlay, perlen, nper, top, hnoflo, hdry, ibound, laytyp, h5_MF_fn, top_array, inputFileMF_fn, lenuni = ppMF.ppMF(MF_ws, rch_input = (h5_rch_fn, 'rch'), rch_dft = 1E-3, wel_input = (h5_rch_fn, 'ETg'), wel_dft = 1E-6)
...
while abs(h_diff[LOOP]) > convcrit:
    ...
    # average for the whole simulated period
    resavg_PERall = np.zeros([nrow,ncol,len(index)], dtype=float)
    # all values for the whole simulated period
    h5_MM_fn = os.path.join(MF_ws,'h5_MM.h5')
    h5_MM = h5py.File(h5_MM_fn, 'w')
    ds_res_PERall = h5_MM.create_dataset(name = 'res_PERall', data = np.zeros([sum(perlen),nrow,ncol,len(index)]), dtype = np.float, chunks = (1,nrow,ncol,len(index)),  compression = 'gzip', compression_opts = 5, shuffle = True)
    # arrays for fluxes in each soil layer
    index_S = {'iEu':0, 'iTu':1,'iSpc':2, 'iRp':3, 'idS':4, 'iS':5}
    # average for the whole simulated period
    resavg_PERall_S = np.zeros([nrow,ncol,_nslmax,len(index_S)], dtype=float)
    # all values for the whole simulated period
    ds_res_PERall_S = h5_MM.create_dataset(name = 'res_PERall_S', data = np.zeros([sum(perlen),nrow,ncol,_nslmax,len(index_S)]), dtype = np.float, chunks = (1,nrow,ncol,_nslmax,len(index_S)),  compression = 'gzip', compression_opts = 5, shuffle = True)
    # to compute net recharge to be exported to MF
    h5_rch = h5py.File(h5_rch_fn, 'w')
    ds_rch = h5_rch.create_dataset(name = 'rch', data = np.zeros([nper,nrow,ncol]), dtype = np.float, chunks = (1,nrow,ncol),  compression = 'gzip', compression_opts = 5, shuffle = True)
    ds_ETg = h5_rch.create_dataset(name = 'ETg', data = np.zeros([nper,nrow,ncol]), dtype = np.float, chunks = (1,nrow,ncol),  compression = 'gzip', compression_opts = 5, shuffle = True)
...
##                        h_MF_tmp[0] = h5_MF['heads'][0,i,j,0]
##                        h_MF_tmp[1:perlen[n]] = h5_MF['heads'][tstart:tend-1,i,j,0]
##                        cbc_tmp = np.zeros([perlen[n]], dtype = float)
##                        cbc_tmp[0] = -h5_MF['cbc'][0,iDRN,i,j,0]
##                        cbc_tmp[1:perlen[n]] = -h5_MF['cbc'][tstart:tend-1,iDRN,i,j,0]
##                    else:
##                        h_MF_tmp = h5_MF['heads'][tstart-1:tend-1,i,j,0]
##                        cbc_tmp = -h5_MF['cbc'][tstart-1:tend-1,iDRN,i,j,0]
                    h_MF_tmp = h5_MF['heads'][tstart:tend,i,j,0]
                    cbc_tmp = -conv_fact*h5_MF['cbc'][tstart:tend,iDRN,i,j,0]/(delr[j]*delc[i])
                    # for the while loop
                    ...
                    for k in range(len(index)):
                        ds_res_PERall[tstart:tend,i,j,k] = results1_temp[:,k]
                    for k in range(len(index_S)):
                        for l in range(nsl_tmp):
                           ds_res_PERall_S[tstart:tend,i,j,l,k] = results2_temp[:,l,k]
                    # output cumulative (1) or averaged by time (sum(perlen))
                    divfact = float(perlen[n])
                    gridoutRF[i,j]      = ds_res_PERall[tstart:tend,i,j,index.get('iRF')].sum()/divfact
                    resavg_PERall[i,j,index.get('iRF')] = resavg_PERall[i,j,index.get('iRF')] + gridoutRF[i,j]
                    gridoutPET[i,j]     = ds_res_PERall[tstart:tend,i,j,index.get('iPET')].sum()/divfact
                    resavg_PERall[i,j,index.get('iPET')] = resavg_PERall[i,j,index.get('iPET')] + gridoutPET[i,j]
                    gridoutPE[i,j]     = ds_res_PERall[tstart:tend,i,j,index.get('iPE')].sum()/divfact
                    resavg_PERall[i,j,index.get('iPE')] = resavg_PERall[i,j,index.get('iPE')] + gridoutPE[i,j]
                    gridoutRFe[i,j]     = ds_res_PERall[tstart:tend,i,j,index.get('iRFe')].sum()/divfact
                    resavg_PERall[i,j,index.get('iRFe')] = resavg_PERall[i,j,index.get('iRFe')] + gridoutRFe[i,j]
                    gridoutdPOND[i,j]    = ds_res_PERall[tstart:tend,i,j,index.get('idPOND')].sum()/divfact
                    resavg_PERall[i,j,index.get('idPOND')] = resavg_PERall[i,j,index.get('idPOND')] + gridoutdPOND[i,j]
                    gridoutPOND[i,j]    = ds_res_PERall[tstart:tend,i,j,index.get('iPOND')].sum()/divfact
                    resavg_PERall[i,j,index.get('iPOND')] = resavg_PERall[i,j,index.get('iPOND')] + gridoutPOND[i,j]
                    gridoutRo[i,j]      = ds_res_PERall[tstart:tend,i,j,index.get('iRo')].sum()/divfact
                    resavg_PERall[i,j,index.get('iRo')] = resavg_PERall[i,j,index.get('iRo')] + gridoutRo[i,j]
                    gridoutEs[i,j]     = ds_res_PERall[tstart:tend,i,j,index.get('iEs')].sum()/divfact
                    resavg_PERall[i,j,index.get('iEs')] = resavg_PERall[i,j,index.get('iEs')] + gridoutEs[i,j]
                    gridoutEg[i,j]       = ds_res_PERall[tstart:tend,i,j,index.get('iEg')].sum()/divfact
                    resavg_PERall[i,j,index.get('iEg')] = resavg_PERall[i,j,index.get('iEg')] + gridoutEg[i,j]
                    gridoutTg[i,j]       = ds_res_PERall[tstart:tend,i,j,index.get('iTg')].sum()/divfact
                    resavg_PERall[i,j,index.get('iTg')] = resavg_PERall[i,j,index.get('iTg')] + gridoutTg[i,j]
                    gridoutRn[i,j]       = ds_res_PERall[tstart:tend,i,j,index.get('iRn')].sum()/divfact
                    resavg_PERall[i,j,index.get('iRn')] = resavg_PERall[i,j,index.get('iRn')] + gridoutRn[i,j]
                    gridoutETg[i,j]       = ds_res_PERall[tstart:tend,i,j,index.get('iETg')].sum()/divfact
                    resavg_PERall[i,j,index.get('iETg')] = resavg_PERall[i,j,index.get('iETg')] + gridoutETg[i,j]
                    gridoutR[i,j]       = ds_res_PERall[tstart:tend,i,j,index.get('iR')].sum()/divfact
                    resavg_PERall[i,j,index.get('iR')] = resavg_PERall[i,j,index.get('iR')] + gridoutR[i,j]
                    gridoutSEEPAGE[i,j]   = ds_res_PERall[tstart:tend,i,j,index.get('iSEEPAGE')].sum()
                    resavg_PERall[i,j,index.get('iSEEPAGE')] = resavg_PERall[i,j,index.get('iSEEPAGE')] + gridoutSEEPAGE[i,j]
                    gridoutMB[i,j]      = ds_res_PERall[tstart:tend,i,j,index.get('iMB')].sum()/divfact
                    resavg_PERall[i,j,index.get('iMB')] = resavg_PERall[i,j,index.get('iMB')] + gridoutMB[i,j]
                    gridoutINTER[i,j]   = ds_res_PERall[tstart:tend,i,j,index.get('iINTER')].sum()/divfact
                    resavg_PERall[i,j,index.get('iINTER')] = resavg_PERall[i,j,index.get('iINTER')] + gridoutINTER[i,j]

                    for l in range(nsl_tmp):
                        gridoutEu[l][i,j] = ds_res_PERall_S[tstart:tend,i,j,l,index_S.get('iEu')].sum()/divfact
                        resavg_PERall_S[i,j,l,index_S.get('iEu')] = resavg_PERall_S[i,j,l,index_S.get('iEu')] + gridoutEu[l][i,j]
                        gridoutTu[l][i,j] = ds_res_PERall_S[tstart:tend,i,j,l,index_S.get('iTu')].sum()/divfact
                        resavg_PERall_S[i,j,l,index_S.get('iTu')] = resavg_PERall_S[i,j,l,index_S.get('iTu')] + gridoutTu[l][i,j]
                        gridoutS[l][i,j] = ds_res_PERall_S[tstart:tend,i,j,l,index_S.get('iS')].sum()/divfact
                        resavg_PERall_S[i,j,l,index_S.get('iSpc')] = resavg_PERall_S[i,j,l,index_S.get('iSpc')] + gridoutS[l][i,j]
                        gridoutRp[l][i,j] = ds_res_PERall_S[tstart:tend,i,j,l,index_S.get('iRp')].sum()/divfact
                        resavg_PERall_S[i,j,l,index_S.get('iRp')] = resavg_PERall_S[i,j,l,index_S.get('iRp')] + gridoutRp[l][i,j]

                    ds_rch[n,i,j] = ds_res_PERall[tstart:tend,i,j,index.get('iR')].sum()/(divfact*1000)
                    ds_ETg[n,i,j] = (ds_res_PERall[tstart:tend,i,j,index.get('iETg')].sum()/(divfact*1000))*(delr[j]*delc[i])
                    # setting initial conditions for the next SP
                    for l in range(nsl_tmp):
                        Si_tmp_array[i,j,l] = ds_res_PERall_S[tend-1,i,j,l,index_S.get('iSpc')]
                        Rpi_tmp_array[i,j,l] = ds_res_PERall_S[tend-1,i,j,l,index_S.get('iRp')]
                        PONDi_tmp_array[i,j] = ds_res_PERall[tend-1,i,j,index.get('iPOND')]
                    ...
                    if SP_d==0:
                        ...
                        outFileHEADS_MF.write('%.6f'%(h5_MF['heads'][tend-1,i,j,0]) + ' ')
                else:
                    ds_res_PERall[:,i,j,:] = hnoflo
                    ds_res_PERall_S[:,i,j,:,:] = hnoflo
...
#        plt.ylabel.Text.position(0.5, -0.5)
    ...
    SP_d, nrow, ncol, delr, delc, nlay, perlen, nper, top, hnoflo, hdry, ibound, laytyp, h5_MF_fn, top_array, inputFileMF_fn, lenuni = ppMF.ppMF(MF_ws, rch_input = (h5_rch_fn, 'rch'), rch_dft = 0.0001, wel_input = (h5_rch_fn, 'ETg'))
...
h5_MF = h5py.File(h5_MF_fn)
h_MF_m = np.ma.masked_values(h5_MF['heads'], hnoflo, atol = 0.09)

top_array_m = np.ma.masked_values(top_array, hnoflo, atol = 0.09)

for i in range(nrow):
    for j in range(ncol):
        h5_MF['cbc'][:,:,i,j,:] = conv_fact*h5_MF['cbc'][:,:,i,j,:]/(delr[j]*delc[i])
...
ds_res_PERall = h5_MM['res_PERall']
ds_res_PERall_S = h5_MM['res_PERall_S']
...
for i in range(nrow):
    for j in range(ncol):
        if ibound[i,j,0]<>0:
            ...
            for l in range(_nslmax):
                outFileEu_PERall[l].write('%.6f'%(resavg_PERall_S[i,j,l,index_S.get('iEu')]/nper) + ' ')
                outFileTu_PERall[l].write('%.6f'%(resavg_PERall_S[i,j,l,index_S.get('iTu')]/nper) + ' ')
                outFileS_PERall[l].write('%.6f'%(resavg_PERall_S[i,j,l,index_S.get('iSpc')]/nper) + ' ')
                outFileRp_PERall[l].write('%.6f'%(resavg_PERall_S[i,j,l,index_S.get('iRp')]/nper) + ' ')
...
for L in range(nlay):
    ...
    DRNmax.append(np.nanmax(-h5_MF['cbc'][:,iDRN,:,:,:]).flatten())
    DRNmin.append(np.nanmin(-h5_MF['cbc'][:,iDRN,:,:,:]).flatten())
    cbcmax.append(np.nanmax(-h5_MF['cbc'][:,:,:,:,:]).flatten())
    cbcmin.append(np.nanmin(-h5_MF['cbc'][:,:,:,:,:]).flatten())
for o in range(len(obs.keys())):
...
flxlst =[ds_res_PERall[:,:,:,index.get('iRF')].sum()/sum(perlen)/ncell,
        -ds_res_PERall[:,:,:,index.get('iINTER')].sum()/sum(perlen)/ncell,
        ds_res_PERall[:,:,:,index.get('iSEEPAGE')].sum()/sum(perlen)/ncell,
        ds_res_PERall_S[:,:,:,:,index_S.get('idS')].sum()/sum(perlen)/ncell,
        ds_res_PERall[:,:,:,index.get('idPOND')].sum()/sum(perlen)/ncell,
        -ds_res_PERall[:,:,:,index.get('iRo')].sum()/sum(perlen)/ncell,
        -ds_res_PERall[:,:,:,index.get('iEs')].sum()/sum(perlen)/ncell,
        -ds_res_PERall_S[:,:,:,:,index_S.get('iEu')].sum()/sum(perlen)/ncell,
        -ds_res_PERall_S[:,:,:,:,index_S.get('iTu')].sum()/sum(perlen)/ncell,
        -ds_res_PERall[:,:,:,index.get('iR')].sum()/sum(perlen)/ncell,
        -ds_res_PERall[:,:,:,index.get('iRn')].sum()/sum(perlen)/ncell,
        -ds_res_PERall[:,:,:,index.get('iEg')].sum()/sum(perlen)/ncell,
        -ds_res_PERall[:,:,:,index.get('iTg')].sum()/sum(perlen)/ncell,
        ds_res_PERall[:,:,:,index.get('iETg')].sum()/sum(perlen)/ncell]
for l in range(nlay):
    for x in range(len(index_cbc)):
        flxlst.append(h5_MF['cbc'][:,index_cbc[x],:,:,l].sum()/sum(perlen)/ncell)
...
if obsCHECK == 1:
    ...
    for o in range(len(obs.keys())):
        ...
        # SATFLOW
        h_satflow = MM_SATFLOW.run(ds_res_PERall[:,i,j,index.get('iR')], float(obs.get(obs.keys()[o])['hi']),float(obs.get(obs.keys()[o])['h0']),float(obs.get(obs.keys()[o])['RC']),float(obs.get(obs.keys()[o])['STO']))
##        # correct heads from MF (1 day delay)
##        h_MF_tmp = np.zeros([sum(perlen)], dtype = float)
##        h_MF_tmp[0] = h5_MF['heads'][0,i,j,0]
##        h_MF_tmp[1:sum(perlen)] = h5_MF['heads'][0:sum(perlen)-1,i,j,0]
        # export ASCII file at piezometers location
        #TODO extract heads at piezo location and not center of cell
        MM_PROCESS.ExportResults(i, j, inputDate, _nslmax, ds_res_PERall, index, ds_res_PERall_S, index_S, -h5_MF['cbc'][:,iDRN,i,j,0], h5_MF['cbc'][:,iRCH,i,j,0], -h5_MF['cbc'][:,iWEL,i,j,0], h_satflow, h5_MF['heads'][:,i,j,0], obs_h[o][0,:], obs_S[o], outFileExport[o], outPESTheads, outPESTsm, obs.keys()[o])
        ...
        if plot_out == 1:
            ...
            MMplot.allPLOT(
            inputDate,
            ds_res_PERall[:,i,j,index.get('iRF')],
            ds_res_PERall[:,i,j,index.get('iPET')],
            ds_res_PERall[:,i,j,index.get('iPE')],
            ds_res_PERall[:,i,j,index.get('iRFe')],
            ds_res_PERall[:,i,j,index.get('idPOND')],
            ds_res_PERall[:,i,j,index.get('iPOND')],
            ds_res_PERall[:,i,j,index.get('iRo')],
            ds_res_PERall_S[:,i,j,0:_nsl[gridSOIL[i,j]-1],index_S.get('iEu')],
            ds_res_PERall_S[:,i,j,0:_nsl[gridSOIL[i,j]-1],index_S.get('iTu')],
            ds_res_PERall[:,i,j,index.get('iEg')],
            ds_res_PERall[:,i,j,index.get('iTg')],
            ds_res_PERall_S[:,i,j,0:_nsl[gridSOIL[i,j]-1],index_S.get('iS')],
            ds_res_PERall_S[:,i,j,0:_nsl[gridSOIL[i,j]-1],index_S.get('idS')],
            ds_res_PERall_S[:,i,j,0:_nsl[gridSOIL[i,j]-1],index_S.get('iSpc')],
            ds_res_PERall_S[:,i,j,0:_nsl[gridSOIL[i,j]-1],index_S.get('iRp')],
            ds_res_PERall[:,i,j,index.get('iSEEPAGE')],
            ds_res_PERall[:,i,j,index.get('iR')],
            ds_res_PERall[:,i,j,index.get('iRn')],
            ds_res_PERall[:,i,j,index.get('iEs')],
            ds_res_PERall[:,i,j,index.get('iMB')],
            h5_MF['heads'][:,i,j,0], h_satflow, obs_h[o][0,:], obs_S[o],
            _Sm[gridSOIL[i,j]-1],
            _Sr[gridSOIL[i,j]-1],
            hnoflo,
            plt_export_fn,
            ...
            flxlst =[ds_res_PERall[:,i,j,index.get('iRF')].sum()/sum(perlen),
                    -ds_res_PERall[:,i,j,index.get('iINTER')].sum()/sum(perlen),
                    ds_res_PERall[:,i,j,index.get('iSEEPAGE')].sum()/sum(perlen),
                    ds_res_PERall_S[:,i,j,:,index_S.get('idS')].sum()/sum(perlen),
                    ds_res_PERall[:,i,j,index.get('idPOND')].sum()/sum(perlen),
                    -ds_res_PERall[:,i,j,index.get('iRo')].sum()/sum(perlen),
                    -ds_res_PERall[:,i,j,index.get('iEs')].sum()/sum(perlen),
                    -ds_res_PERall_S[:,i,j,:,index_S.get('iEu')].sum()/sum(perlen),
                    -ds_res_PERall_S[:,i,j,:,index_S.get('iTu')].sum()/sum(perlen),
                    -ds_res_PERall[:,i,j,index.get('iR')].sum()/sum(perlen),
                    -ds_res_PERall[:,i,j,index.get('iRn')].sum()/sum(perlen),
                    -ds_res_PERall[:,i,j,index.get('iEg')].sum()/sum(perlen),
                    -ds_res_PERall[:,i,j,index.get('iTg')].sum()/sum(perlen),
                    ds_res_PERall[:,i,j,index.get('iETg')].sum()/sum(perlen)]
            for l in range(nlay):
                for x in range(len(index_cbc)):
                    flxlst.append(h5_MF['cbc'][:,index_cbc[x],i,j,l].sum()/sum(perlen))
...
if plot_out == 1:
    ...
    TSlst = []
    TS = 0
    while TS < len(h5_MF['heads']):
        TSlst.append(TS)
        TS = TS + plot_freq
    TSlst.append(len(h5_MF['heads'])-1)
    for TS in TSlst:
        # plot heads [m]
        V=[]
        for L in range(nlay):
