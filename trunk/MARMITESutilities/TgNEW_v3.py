...
daynum= 350
#veg param
subtitle_list = ['major phreatophytic behavior', 'intermediate phreatophytic behavior', 'minor to null phreatophytic behavior']
veg_list = ['riparian', 'quercus', 'grass']
f_lst = [0.5, 0.102, 0.5]#[0.10, 0.217336864608619, 0.30]
s_lst = [1.00, 22.15, 1.00]#[20, 28.26674282, 35]
kTs_max_lst = [0.95, 0.955, 0.10]#[0.9, 0.712676596, 0.35]
kTs_min_lst = [0.90, 0.136, 0.05]#[0.6, 0.19772923, 0.05]
# kTs obs
...
title = plt.title(r'a) - $k_{T_s}$ function of $\theta_{norm}$', fontsize = 12)
...
m = []
texts = []
for l, (s, s_inv, f, kTs_max, kTs_min, v, plt_numb) in enumerate(zip(s_inv_lst, s_lst, f_lst, kTs_max_lst, kTs_min_lst, veg_list, plt_numb_lst)):
    kTs = kTs_max - (kTs_max-kTs_min)/(1 + np.exp((thetanorm - f)/s))
    plts += ax1.plot(thetanorm, kTs, linesTg.__next__(), color = cTg, label = '%s'%v)
...
for l, (v,c,month) in enumerate(zip(kTs_source,kTs_color,month_lst)):
        plts += ax1.plot(theta_obs[l],kTs_obs[l], 'o', label = v, color = c, markersize = 5)
...
linesTg = itertools.cycle(['-.', '-', '--', ':'])
for j, (s, f, kTs_max, kTs_min, titlesuffix, v, plt_numb) in enumerate(zip(s_inv_lst, f_lst, kTs_max_lst, kTs_min_lst, subtitle_list, veg_list, plt_numb_lst)):
