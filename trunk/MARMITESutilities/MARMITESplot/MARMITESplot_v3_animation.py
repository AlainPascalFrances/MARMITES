...
def plotLAYER(timesteps, Date, JD, ncol, nrow, nlay, nplot, V, cmap, CBlabel, msg, plt_title, MM_ws, interval_type = 'arange', interval_diff = 1, interval_num = 1, Vmax = 0, Vmin = 0, fmt = '%.2f', contours = False, ntick = 1, axisbg = 'silver', points  = None, mask = None):
    ...
    for i, day in enumerate(timesteps):
        ...
        for L in range(nplot):
            if mask == None:
                ...
                Vtmp = np.ma.masked_array(V[day,:,:,L], mask[L])
        ...
        if max(x) > max(y):
            ...
            CBorient = 'vertical'
        CB = fig.colorbar(ims[0][0], extend='both', ticks = ticks, format = fmt, cax = cax,  orientation = CBorient)
    ...
    if len(timesteps)>1:
        ani = animation.ArtistAnimation(fig, ims, interval=10*(timesteps[1]-timesteps[0]), repeat_delay=500, blit=True)
        ani.save(os.path.join(MM_ws, '_plt_%s_mov.mp4' % plt_title))
    plt.close('all')

    del fig, ax, timesteps, ncol, nrow, nlay, nplot, V, cmap, CBlabel, msg, plt_title, MM_ws, interval_type, interval_diff, interval_num, Vmax, Vmin, fmt
