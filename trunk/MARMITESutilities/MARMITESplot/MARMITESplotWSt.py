...
def allPLOT(DateInput, P, PET, PE, Pe, dPOND, POND, Ro, Eu, Tu, Eg, Tg, S, dS, Spc, Rp, SEEPAGE, R, Rn, Es, MB, h_MF, h_SF, hobs, Sobs, Sm, Sr, hnoflo, plt_export_fn, plt_title, colors_nsl, hmax, hmin):
    ...
    fig = plt.figure(num=None, figsize=(8.5,15), dpi = 30)    #(2*8.27, 2*11.7), dpi=30)
    ...
    lbl_Eu.append('PE')
    lbl_Eu.append('E_tot')
    lbl_Eu.append('Eu')   #('Eu_tot')
    lbl_Tu.append('PET')
    lbl_Tu.append('T_tot')
    lbl_Tu.append('Tu') #('Tu_tot')
    Sobs_m = []
    Eu1 = []
    Tu1 = []
    dS1 = []
...
#    lbl_Rp.append('Rn')
    ...
    plt.xlim(DateInput[0]-10,DateInput[nts-1]+10)
...
#        ax6.plot_date(DateInput, y, linestyle='-', linewidth = 0.5, marker='o', markersize=5, color = color, label=lbl)
#    for l, (y, color, lbl) in enumerate(zip(Spc1, colors_nsl, lbl_Spc)) :
#        ax6.plot_date(DateInput, y, '-', color = color, label=lbl)
    ax6.plot_date(DateInput, Spc1[0], '-', color = colors_nsl[0], label=lbl_Spc[0])
    # x axis
    plt.xlim(DateInput[0]-1,DateInput[len(P)-1]+1)
    # y axis
    ...
    plt.legend((r'hsim_MF'), loc=0, labelspacing=lblspc, markerscale=mkscale)
    leg = plt.gca().get_legend()
    ltext  = leg.get_texts()
    plt.setp(ltext, fontsize=8 )
