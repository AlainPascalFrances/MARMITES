...
kT_max_lst = [0.95, 0.956, 0.870, 0.10]#[0.9, 0.712676596, 0.35]
kT_min_lst = [0.90, 0.130, 0.044, 0.05]#[0.6, 0.19772923, 0.05]
# kTs obs
...
plt.ylabel(r'$k_T$')  #, color = cTg)
#"plt.setp(ax1.get_yticklabels(), color = cTg)
ax1.yaxis.set_major_formatter(mpl.ticker.FormatStrFormatter('%.1f'))
#ax1.text(theta_wp-0.015, kT_min, '${k_{T_s}}_{min}$', horizontalalignment='center', verticalalignment='center', fontsize = 10)
#ax1.text(phi+0.015, kT_max, '${k_{T_s}}_{max}$', horizontalalignment='center', verticalalignment='center', fontsize = 10)
...
linesTg = itertools.cycle(['-.', '-', '--', ':'])
for j, (s, f, kT_max, kT_min, titlesuffix, v, plt_numb) in enumerate(zip(s_inv_lst, f_lst, kT_max_lst, kT_min_lst, subtitle_list, veg_list, plt_numb_lst)):
    if j!=1:
...
#plt.show()
img_path = 'Tg_functionNEW5_%s'% (datetime.date.today().strftime("%Y%m%d"))
