...
ThickSoil = 1000.0
PT = 3.5
kT_min_lst =  [0.1, 0.8, 0.1]  # [0.1, 0.1, 0.1]
kT_max_lst =  [0.9, 0.9, 0.2]  # [0.8, 1.00, 0.5]
...
for j, (kT_min, kT_max, titlesuffix, plt_numb) in enumerate(zip(kT_min_lst, kT_max_lst, titlesuffix_lst, plt_numb_lst)):    
    ...
    ax2a.text(n_x, n_y, r'$n$ = %.1f, (${k_T}_{min}$, ${k_T}_{max}$) = (%.1f, %.1f)' % (0.5, kT_min, kT_max), horizontalalignment='right', verticalalignment='center', transform=ax2a.transAxes, bbox=dict(facecolor='white'), fontsize = 10)
    ...
    #ltext  = leg.get_texts()
    #plt.setp(ltext, fontsize=10)
    ax3a.text(n_x, n_y, r'$n$ = %.1f, (${k_T}_{min}$, ${k_T}_{max}$) = (%.1f, %.1f)' % (2.0, kT_min, kT_max), horizontalalignment='right', verticalalignment='center', transform=ax3a.transAxes, bbox=dict(facecolor='white'), fontsize = 10)

plt.subplots_adjust(left=0.075, bottom=0.05, right=0.95, top=0.95, wspace=0.25, hspace=0.3)    
plt.show()
img_path = 'Tg_function'
