...
ET_MM  = data[:,2].astype(np.float)
RF     = data[:,3].astype(np.float)
ET_EC  = data[:,4].astype(np.float)
Ro_MM  = data[:,5].astype(np.float)
Ro_obs = data[:,6].astype(np.float)
...
#####################################
hnoflo = 99999.99
EC = np.where(~np.isnan(ET_EC),ET_EC,hnoflo)
MM = np.where(~np.isnan(ET_MM),ET_MM,hnoflo)
print 'ET_MM vs ET_EC\nrmse, rsr, nse, r'
print '%.2f, %.2f, %.2f, %.2f' % compCalibCrit(EC, MM, hnoflo)
...
plt_suptitle = 'ET EC vs ET MM + RF' 
